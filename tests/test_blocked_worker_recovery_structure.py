...
def test_recovery_always_requests_transit_before_movement_and_restores_before_success():
    control=read("scripts/control/BlockedWorkerRecoveryControl.lua")
    ...
    assert "RECOVERY_ANCHOR_REACHED_RESTORED_AND_HANDED_BACK" in control
    ...
    assert 'configurationPolicy="ALWAYS_REQUEST_TRANSIT_THEN_RESTORE"' in read("scripts/runtime/Runtime.lua")
...
def test_recovery_control_failure_remains_pre_semantic_and_configuration_debt_is_not_forgotten():
    runtime=read("scripts/runtime/Runtime.lua")
    ...
    assert "configurationMechanism:clear" not in control
    assert "requestCachedTransitRestore" in control
