...
load("scripts/commitment/ObstructionRelocationCommitmentLifecycle.lua")
load("scripts/commitment/BlockedWorkerRecoveryCommitmentLifecycle.lua")
...
load("scripts/candidates/BlockedWorkerRecoveryCandidateSupport.lua")
load("scripts/candidates/LiveTrafficCandidateSupport.lua")
load("scripts/decision/ProspectivePortfolioDecisionPolicy.lua")
load("scripts/candidates/ProspectiveDecisionPortfolioSupport.lua")
load("scripts/constraints/ConstraintEvidence.lua")
load("scripts/constraints/evaluators/RepresentationFitnessConstraint.lua")
load("scripts/constraints/evaluators/ResponsibilityCompatibilityConstraint.lua")
...
test("Operational Picture rejects Decision semantics",function()
    expectError(function() OuttaMyWay.OperationalPicture.new({identity="OP-X",epoch=1,observationSnapshotId="OS-X",situations={},currentPairAssessmentScope={},identities={},currentSpace={},futureSpace={},demand={committedDemand={},potentialDemand={},temporarySlack={}},responsibilityRelations={},uncertainty={},representationFitness={},provenance={},controlOutcomeEvidence={},candidateSupportEvidence={selectedCandidateId="CA-X"},commitmentContext={}}) end)
end)
...
test("sealed fixture produces one Operation Situation and current Pair Assessment Scope",function()
    local runtime=newPictureRuntime(); local result=runtime:processSealedObservation(pictureFixture(1))
    equal(#result.operation.activeOperationIds,1); equal(#result.picture.situations,1); equal(#result.picture.currentPairAssessmentScope,1)
    local pair=result.picture.currentPairAssessmentScope[1]
    equal(pair.relationshipStatus,"POSITIVE"); equal(pair.futureSpaceStatus,"POSITIVE"); equal(pair.relationship,"FUTURE_SPACE_CONVERGENCE")
    equal(pair.provenance.ephemeral,true); equal(pair.provenance.persistentPairHistory,false)
end)

test("current pair scope becomes unresolved when positive evidence is temporarily absent",function()
    local runtime=newPictureRuntime(); local first=runtime:processSealedObservation(pictureFixture(1))
    local firstPair=first.picture.currentPairAssessmentScope[1]
    local second=runtime:processSealedObservation(pictureFixture(2,{interactions={}})); local pair=second.picture.currentPairAssessmentScope[1]
    equal(#second.picture.currentPairAssessmentScope,1); equal(pair.pairReferenceKey,firstPair.pairReferenceKey); equal(pair.episodeSignature,firstPair.episodeSignature)
    equal(pair.relationshipStatus,"UNRESOLVED"); equal(pair.relationship,"UNRESOLVED"); equal(pair.currentSpaceStatus,"UNRESOLVED"); equal(pair.futureSpaceStatus,"UNRESOLVED")
    equal(pair.evidence.currentPositiveProvenance,nil); equal(pair.evidence.futurePositiveProvenance,nil)
end)
...
test("Job Episode replacement rebuilds current pair scope without stale state",function()
    ...
    local renewed=runtime:processSealedObservation(pictureFixture(4,{evidenceB={sourceJobToken="job-B2",jobPresent=true,aiControlled=true,aiActive=true}})); equal(renewed.picture.currentPairAssessmentScope[1].relationshipStatus,"POSITIVE")
end)
...
test("incomplete membership evidence does not end an Operation",function()
    ...
    local found=false; for _,u in OuttaMyWay.ValueRecord.ipairs(second.picture.uncertainty) do if u.class=="OPERATION_MEMBERSHIP_INCOMPLETE" then found=true end end
...
test("duplicate current interaction evidence collapses to one pair scope",function()
    ...
    equal(#picture.currentPairAssessmentScope,1); equal(picture.currentPairAssessmentScope[1].relationshipStatus,"POSITIVE"); equal(#picture.currentPairAssessmentScope[1].evidence.sourceInteractionReferenceKeys,2)
end)

test("fresh current intersection takes precedence in current pair assessment",function()
    ...
    local runtime=newPictureRuntime(); local pair=runtime:processSealedObservation(pictureFixture(1,{interactions=interactions})).picture.currentPairAssessmentScope[1]
    equal(pair.relationshipStatus,"POSITIVE"); equal(pair.currentSpaceStatus,"POSITIVE"); equal(pair.futureSpaceStatus,"POSITIVE"); equal(pair.relationship,"CURRENT_SPACE_INTERACTION")
end)
...
test("Situation Assessment preserves unavailable sources and non-fit representations",function()
    ...
    local classes={}; for _,u in OuttaMyWay.ValueRecord.ipairs(picture.uncertainty) do classes[u.class]=true end
...
local function decisionPicture(specifications,options)
    ...
    return OuttaMyWay.OperationalPicture.new({
        ...
        currentPairAssessmentScope={{pairReferenceKey="pair:decision",operationId="OR-1",subjectAssemblyId="AS-00001",otherAssemblyId="AS-00002",subjectJobEpisodeId="JE-1",otherJobEpisodeId="JE-2",episodeSignature="JE-1|JE-2",relationshipStatus="UNRESOLVED",relationship="UNRESOLVED",currentSpaceStatus="UNRESOLVED",futureSpaceStatus="UNRESOLVED",evidence={negativeClearanceAuthority=false},provenance={ephemeral=true,persistentPairHistory=false}}},
...
test("Bounded Authority accepts Candidate-supplied composition identity without EC convention",function()
    ...
    local grant=activeGrant(runtime,current,commitment,token,{kind="REGULATION_LEASE",vehicleReferenceKey="ref:ba-a",ownerTag="ACTION_SPACE_REGULATION",maxSpeedKmh=5,governingPurpose="BA_TEST_REGULATION"})
    ...
    local request=OuttaMyWay.ControlRequest.new({
        ...
        assemblyId=token.assemblyId,
        capability="REGULATE_SPEED",
        target={kind="REGULATION_LEASE",operation="APPLY",vehicleReferenceKey="ref:ba-a",ownerTag="ACTION_SPACE_REGULATION",maxSpeedKmh=5,governingPurpose="BA_TEST_REGULATION"},
        authorityToken=token.identity,
...
test("Rejected fresh Bounded Authority grant is released before return",function()
    ...
    local result=dispatcher:_continueActionSpaceRegulationInitial({epoch=220},boundedAuthorityEvaluated(candidate),candidate,actionSpaceBridge(),{
        commitment=commitment,authorityToken=token,authorityAcquired=true,currentResponsibility=current
    })
...
test("Rejected Bounded Authority update removes successor while predecessor remains current",function()
    ...
    local result=dispatcher:_updateActionSpaceRegulationEnvelope({epoch=220},boundedAuthorityEvaluated(candidate),candidate,lease,{currentClosing={separationM=80}},"RELATIONSHIP_REMAINS")
...
test("every candidate receives every mandatory constraint verdict",function()
    ...
    equal(#result.verdictSet.mandatoryConstraintIds,4); equal(#result.verdicts,8); equal(result.verdictSet.complete,true)
end)

test("FAIL and UNRESOLVED mandatory verdicts cannot remain viable",function()
    local failed=candidateSpec("failed","HOLD",0)
    failed.representationFitness={requirements={{representationId="REP-FAIL",acceptedStates={"CURRENTLY_FIT"}}}}
    failed.evidenceBasis.effectiveActuationComposition={identity="EC-FAIL",epoch=1,relevantAssemblyIds={"AS-00001","AS-00002"},entries={{assemblyId="AS-00001",commitmentId="CM-FAIL",capability="HOLD",effectClass="HOLD",progressActuation=false}}}
    local unresolved=candidateSpec("unresolved","REGULATE_SPEED",1)
    unresolved.representationFitness={requirements={{representationId="REP-MISSING",acceptedStates={"CURRENTLY_FIT"}}}}
    unresolved.evidenceBasis.effectiveActuationComposition={identity="EC-UNRESOLVED",epoch=1,relevantAssemblyIds={"AS-00001","AS-00002"},entries={{assemblyId="AS-00001",commitmentId="CM-UNRESOLVED",capability="REGULATE_SPEED",progressActuation=true}}}
    local pass=candidateSpec("pass","CONTINUE_UNCHANGED",5)
    local picture=decisionPicture({failed,unresolved,pass},{representationFitness={{representationId="REP-FAIL",assemblyId="AS-00001",question="HOLD",assessmentHorizon=5,state="STRUCTURALLY_INVALID",claimPermissions={},coverage={complete=true,conservative=true},uncertainty={},validityDependencies={},provenance={}}}})
    local result=newDecisionRuntime():evaluateSealedOperationalPicture(picture)
    equal(findVerdict(result,candidateByCapability(result,"HOLD").identity,"REPRESENTATION_FITNESS").result,"FAIL")
    local unresolvedCandidate=candidateByCapability(result,"REGULATE_SPEED")
    equal(findVerdict(result,unresolvedCandidate.identity,"REPRESENTATION_FITNESS").result,"UNRESOLVED")
...
test("comparison cost is applied only after mandatory admissibility",function()
    ...
    cheap.representationFitness={requirements={{representationId="REP-FAIL",acceptedStates={"CURRENTLY_FIT"}}}}
    cheap.evidenceBasis.effectiveActuationComposition={identity="EC-CHEAP-FAIL",epoch=1,relevantAssemblyIds={"AS-00001","AS-00002"},entries={{assemblyId="AS-00001",commitmentId="CM-CHEAP-FAIL",capability="HOLD",effectClass="HOLD",progressActuation=false}}}
    ...
    local picture=decisionPicture({cheap,expensive},{representationFitness={{representationId="REP-FAIL",assemblyId="AS-00001",question="HOLD",assessmentHorizon=5,state="STRUCTURALLY_INVALID",claimPermissions={},coverage={complete=true,conservative=true},uncertainty={},validityDependencies={},provenance={}},{representationId="REP-A",assemblyId="AS-00001",question="SPEED",assessmentHorizon=5,state="CURRENTLY_FIT",claimPermissions={"SPEED"},coverage={complete=true,conservative=true},uncertainty={},validityDependencies={},provenance={}}}})
    local result=newDecisionRuntime():evaluateSealedOperationalPicture(picture)
...
local function headOnSupportPicture(options)
    ...
    return OuttaMyWay.OperationalPicture.new({
        ...
        currentPairAssessmentScope={{pairReferenceKey="vehicle-root:101|vehicle-root:201",operationId="OR-1",subjectAssemblyId="AS-00001",otherAssemblyId="AS-00002",subjectReferenceKey="vehicle-root:101",otherReferenceKey="vehicle-root:201",subjectJobEpisodeId="JE-A",otherJobEpisodeId="JE-B",episodeSignature="JE-A|JE-B",relationshipStatus="POSITIVE",relationship=currentInteraction and "CURRENT_SPACE_INTERACTION" or "FIELD_BOUNDED_FUTURE_SPACE_INTERSECTION",currentSpaceStatus=currentInteraction and "POSITIVE" or "UNRESOLVED",futureSpaceStatus="POSITIVE",currentInteractionEvidencePresent=true,evidence={sourceInteractionReferenceKeys={"vehicle-root:101|vehicle-root:201"},negativeClearanceAuthority=false},provenance={source="head-on-test",ephemeral=true,persistentPairHistory=false}}},
...
test("fully failed complete space produces explicit SETTLE non-intervention",function()
    local candidate=candidateSpec("failed","HOLD",1)
    candidate.representationFitness={requirements={{representationId="REP-FAIL",acceptedStates={"CURRENTLY_FIT"}}}}
    candidate.evidenceBasis.effectiveActuationComposition={identity="EC-SETTLE-FAIL",epoch=1,relevantAssemblyIds={"AS-00001","AS-00002"},entries={{assemblyId="AS-00001",commitmentId="CM-SETTLE-FAIL",capability="HOLD",effectClass="HOLD",progressActuation=false}}}
    local picture=decisionPicture({candidate},{representationFitness={{representationId="REP-FAIL",assemblyId="AS-00001",question="HOLD",assessmentHorizon=5,state="STRUCTURALLY_INVALID",claimPermissions={},coverage={complete=true,conservative=true},uncertainty={},validityDependencies={},provenance={}}}})
    local result=newDecisionRuntime():evaluateSealedOperationalPicture(picture)
    equal(findVerdict(result,result.candidates[1].identity,"REPRESENTATION_FITNESS").result,"FAIL")
...
test("SETTLE directive cannot contradict canonical Governing Basis event",function()
    ...
    local failed=candidateSpec("failed","HOLD",1)
    failed.representationFitness={requirements={{representationId="REP-FAIL",acceptedStates={"CURRENTLY_FIT"}}}}
    failed.evidenceBasis.effectiveActuationComposition={identity="EC-SETTLE-MAPPING",epoch=1,relevantAssemblyIds={"AS-00001","AS-00002"},entries={{assemblyId="AS-00001",commitmentId="CM-SETTLE-MAPPING",capability="HOLD",effectClass="HOLD",progressActuation=false}}}
    local picture=decisionPicture({failed},{commitmentContext={{commitmentId=admitted.commitment.identity,settlementDirective={eventKind="OBJECTIVE_SATISFIED",intendedTerminalDisposition="FAILED",terminalCause="OBJECTIVE_FAILED"}}},representationFitness={{representationId="REP-FAIL",assemblyId="AS-00001",question="HOLD",assessmentHorizon=5,state="STRUCTURALLY_INVALID",claimPermissions={},coverage={complete=true,conservative=true},uncertainty={},validityDependencies={},provenance={}}}})
    local result=runtime:evaluateSealedOperationalPicture(picture)
    equal(result.decision.commitmentAction,"SETTLE")
    expectError(function() runtime.decisionCommitmentBoundary:apply(picture,result) end)
    equal(runtime.commitments:get(admitted.commitment.identity).state,"ACTIVE")
...
test("closing motion alone leaves current pair relationship unresolved",function()
    withFakeLiveGlobals(function(mission,a,b,positions,jobA,jobB,field,farmland,directions)
        directions[201]={0,-1}; a.lastSpeedReal=0.003; b.lastSpeedReal=0.003
        local runtime=OuttaMyWay.Runtime.new(); runtime:initialize(); local raw=runtime.liveObservationSource:capture(mission,10)[1]; local pair=raw.diagnostics.pairDiagnostics[1]
        equal(pair.principalOutcome,"CURRENT_INTERACTION_UNRESOLVED"); if pair.closingRate<=0 then error("closing motion evidence lost") end; equal(pair.interactionEvidenceEmitted,false)
        local processed=runtime:processSealedObservation(raw); equal(#processed.picture.currentPairAssessmentScope,1); equal(processed.picture.currentPairAssessmentScope[1].relationshipStatus,"UNRESOLVED"); equal(processed.picture.diagnostics.pairPipeline[1].currentPairScopePresent,true)
    end)
end)
...
test("filtered current-space footprint positive reaches current pair scope when scalar radius is missing",function()
    withFakeLiveGlobals(function(mission,a,b,positions,jobA,jobB,field,farmland,directions,strategies)
        local saved={
        ...
        equal(processed.picture.currentPairAssessmentScope[1].currentSpaceStatus,"POSITIVE")
        equal(processed.picture.currentPairAssessmentScope[1].evidence.currentPositiveProvenance.authority,"POSITIVE_INTERACTION_ONLY")
        equal(processed.picture.currentPairAssessmentScope[1].evidence.currentPositiveProvenance.negativeClearanceAuthority,false)
...
test("field-bounded Future Space reaches current pair scope after legacy predictor removal", function()
    withFakeLiveGlobals(function(mission,a,b,positions,jobA,jobB,field,farmland,directions)
        local saved={
            getNumOfChildren=getNumOfChildren,getChildAt=getChildAt,getName=getName,localToWorld=localToWorld,
            getShapeGeometryBoundingSphere=getShapeGeometryBoundingSphere,getShapeBoundingSphere=getShapeBoundingSphere,
        ...
        equal(processed.picture.currentPairAssessmentScope[1].futureSpaceStatus,"POSITIVE")
        equal(processed.picture.currentPairAssessmentScope[1].evidence.futurePositiveProvenance.source,"FIELD_BOUNDED_FUTURE_SPACE_POSITIVE")
        equal(processed.picture.currentPairAssessmentScope[1].evidence.futurePositiveProvenance.negativeClearanceAuthority,false)
        getNumOfChildren=saved.getNumOfChildren; getChildAt=saved.getChildAt; getName=saved.getName; localToWorld=saved.localToWorld
        getShapeGeometryBoundingSphere=saved.getShapeGeometryBoundingSphere; getShapeBoundingSphere=saved.getShapeBoundingSphere
        getShapeWorldBoundingSphere=saved.getShapeWorldBoundingSphere; getIsCompoundChild=saved.getIsCompoundChild
    end)
...
test("Situation Assessment publishes field-bounded Future Space relationship as Knowledge only", function()
    ...
    equal(#result.picture.currentPairAssessmentScope,1)
    equal(result.picture.currentPairAssessmentScope[1].relationshipStatus,"UNRESOLVED")
...
test("rejected Encounter lifecycle modules are absent from the active harness", function()
    equal(rawget(OuttaMyWay,"EncounterRegistry"),nil)
    equal(rawget(OuttaMyWay,"TransitionHud"),nil)
...
test("Reverse Reposition preserves GIANTS AI reverser reference frame",function()
    ...
    AIVehicleUtil={
        ...
        getAIToolReverserDirectionNode=function(vehicle) return 903 end
    }
    getWorldTranslation=function(node)
        if node==900 then return 10,0,0 end
        if node==901 then return 8,0,2 end
        if node==903 then return 6,0,4 end
        return 0,0,0
    end
    worldDirectionToLocal=function(node,x,y,z) return x,y,z end
    ...
    local vehicle={
        rootNode=900,
        getAISteeringNode=function(self) return 900 end,
        getAIReverserNode=function(self) return 901 end
    }
    local authority=OuttaMyWay.NativeDriveMechanism.new()

    equal(authority:setReposition(vehicle,0,0,8,0.5,false),true)
    local state=authority:getState(vehicle)
    equal(state.repositionReferenceNode,901)
    equal(state.repositionReferenceNodeSource,"AI_REVERSER_NODE")
    equal(state.reverseReferenceDistinctFromSteering,true)
    equal(state.toolReverserDirectionNode,903)
    equal(state.toolReverserDirectionNodeSource,"AI_TOOL_REVERSER_DIRECTION_NODE")

    AIVehicleUtil.driveToPoint(vehicle,16,1,true,true,0,1,25)
    equal(calls[#calls].moveForwards,false)
    equal(math.abs(calls[#calls].lx)<0.0001,true)
    equal(math.abs(calls[#calls].lz+1)<0.0001,true)
    equal(math.abs(authority:getState(vehicle).lastRemainingM-10)<0.0001,true)
...
test("Regulation-Hold Boundary maps zero cap to GIANTS no-drive permission", function()
    ...
    equal(calls[#calls].allowed,true)
    equal(calls[#calls].maxSpeed,10.0)

    equal(authority:setRegulationLease(vehicle,1.0,"TRAFFIC"),true)
    AIVehicleUtil.driveToPoint(vehicle,16,1,true,true,0,1,25.0)
    equal(calls[#calls].allowed,true)
    equal(calls[#calls].maxSpeed,1.0)
...
test("Cooperative Passage: Action-Space Regulation crosses Candidate Decision Commitment Control and succeeds into same-Commitment Passage",function()
    ...
    local admitted=runtime:dispatchEvaluatedOperationalPicture(supported,evaluated)
    equal(admitted.status,"ACCEPTED"); equal(admitted.actionSpaceRegulation,true)
    equal(admitted.currentResponsibility.kind,"REGULATION")
    equal(regulationRequests[#regulationRequests].target.ownerTag,"ACTION_SPACE_REGULATION")
    ...
    local envelopeStatus=runtime.regulationBoundedAuthority:getActionSpaceRegulationStatus()
    ...
    equal(regulationRequests[2].target.ownerTag,"ACTION_SPACE_REGULATION")
    equal(runtime.obligations:get(actionObligation.identity).status,"SETTLED")
...
for _,failure in ipairs({"TARGET","LEASE","OBLIGATION","PARTICIPANTS","SUCCESSOR_OBLIGATION","REVISION","PHYSICAL_CLEANUP","CLEANUP"}) do
    test("Follower Passage refusal preserves sole predecessor responsibility: "..failure,function()
        ...
        elseif failure=="PHYSICAL_CLEANUP" then
            runtime.regulationBoundedAuthority.regulationControl.executeControlRequest=function(self,request,candidate)
                if request.target~=nil and request.target.operation=="RELEASE" then
                    return false,"INJECTED_PHYSICAL_CLEANUP_RELEASE_REFUSAL"
                end
                return true,"ACCEPTED"
            end
            runtime.regulationBoundedAuthority.regulationControl.clearRegulationLeaseByReference=function()
...
test("Action-Space responsibility replacement preflight refusal leaves retained substrate unchanged",function()
    ...
    local leaseAfter=runtime.regulationBoundedAuthority:getActionSpaceRegulationStatus()
...
test("Action-Space initial revalidation preserves its explicit Regulation identity",function()
    local runtime=autonomousHeadOnRuntime()
    local authority=runtime.responsibilityTransitionAuthority
    local evaluated={decision={selectedCandidateId="CA-TEST"},candidates={{identity="CA-TEST",evidenceBasis={actionSpaceRegulationBridge={conflictIdentity="CONFLICT-TEST"}}}}}
...
test("Resolution-Space Progression Envelope tightens prospectively as ordinary space is consumed",function()
    ...
    function capability:getVehicleControlObservationByReference(referenceKey)
        return {mode="REGULATE",ownerTag="ACTION_SPACE_REGULATION",regulationSpeedKmh=25,actualSpeedKmh=self.actualSpeedKmh,driveCalls=1,lastOutputMaxSpeed=25,lastInputForward=true}
    end
    runtime:setRegulationControl(capability)
...
test("Transient reverse non-closing evidence retains Cooperative Passage obligation but quiesces Action-Space Regulation actuation",function()
    ...
    equal(runtime.regulationBoundedAuthority:getActionSpaceRegulationStatus().actuationActive,false)
    equal(runtime.commitments:get(commitmentId).state,"ACTIVE")
    equal(#runtime.obligations:openForOwner(commitmentId),1)
...
test("Cooperative Passage Established Conflict crosses Candidate Decision Commitment and central Control dispatch",function()
    ...
    equal(dispatched.requests[1].target.kind,"COOPERATIVE_PASSAGE"); equal(dispatched.requests[2].target.kind,"COOPERATIVE_PASSAGE")
    equal(dispatched.candidate.evidenceBasis.cooperativePassageBridge.architecture,"COOPERATIVE_PASSAGE")
...
local function passageLegRuntime(runtime,assemblyA,assemblyB,episodeA,episodeB)
    if runtime==nil then runtime=OuttaMyWay.Runtime.new(); runtime:initialize() end
    assemblyA=assemblyA or "AS-A"; assemblyB=assemblyB or "AS-B"
    episodeA=episodeA or "JE-A"; episodeB=episodeB or "JE-B"
    local commitment=runtime.commitmentAdmission:admit({
        objective={kind="COOPERATIVE_PASSAGE"},
        governingBasis={responsibilityKey="cooperative-passage:LEG-LIFECYCLE",dependentPairReferenceKey="pair:LEG-LIFECYCLE",dependentJobEpisodeIds={episodeA,episodeB},sourceIntentIds={episodeA,episodeB}},
    ...
    local composition=OuttaMyWay.EffectiveActuationComposition.create({
        identity="EC-LEG-LIFECYCLE",epoch=1,relevantAssemblyIds={assemblyA,assemblyB},
        entries={
            {assemblyId=assemblyA,commitmentId=commitment.identity,capability="REPOSITION",effectClass="MOVE",progressActuation=true},
            {assemblyId=assemblyB,commitmentId=commitment.identity,capability="REPOSITION",effectClass="MOVE",progressActuation=true}
        }
    })
    ...
    runtime.responsibilityTransitionAuthority.resolutionsByCommitmentId[commitment.identity]=OuttaMyWay.ResolutionCommitment.new({
        ...
        beneficiaryAssemblyIds={assemblyA,assemblyB},controlledSubjectAssemblyIds={assemblyA,assemblyB},
    ...
    for _,token in OuttaMyWay.ValueRecord.ipairs(runtime.authorities:tokensForCommitment(commitment.identity)) do
        local grant,reason=runtime.boundedAuthority:authorize({
            ...
            target={kind="COOPERATIVE_PASSAGE",assemblyId=token.assemblyId},authorityToken=token.identity,
...
test("Cooperative Passage Leg: Issue 51 ended already handed-back participant does not settle survivor leg",function()
    ...
    local collapsed=OuttaMyWay.LiveTrafficCommitmentLifecycle.applyCooperativePassageParticipantLosses(runtime,{endedEpisodeIds={"JE-A"},observationSnapshotId="OBS-51"},{identity="OBS-51"})
    equal(#collapsed,0)
    equal(runtime.commitments:get(commitmentId).state,"ACTIVE")
    equal(runtime.obligations:openForOwner(commitmentId)[1].basis.assemblyId,"AS-B")
    equal(runtime.authorities:ownerOf("AS-B"),commitmentId)
...
test("Cooperative Passage Leg: both dependent Passage Leg Job Episodes ending in one observation vacates both legs",function()
    ...
    local collapsed=OuttaMyWay.LiveTrafficCommitmentLifecycle.applyCooperativePassageParticipantLosses(runtime,{endedEpisodeIds={"JE-A","JE-B"},observationSnapshotId="OBS-BOTH"},{identity="OBS-BOTH"})
    equal(#collapsed,2)
...
test("Cooperative Passage Leg: already terminal A and live B ending in one observation only vacates B",function()
    ...
    runtime.jobEpisodes.records["JE-A"]={identity="JE-A",assemblyId="AS-A",status="ENDED"}
    runtime.jobEpisodes.records["JE-B"]={identity="JE-B",assemblyId="AS-B",status="ENDED"}
    local collapsed=OuttaMyWay.LiveTrafficCommitmentLifecycle.applyCooperativePassageParticipantLosses(runtime,{endedEpisodeIds={"JE-A","JE-B"},observationSnapshotId="OBS-MIXED-END"},{identity="OBS-MIXED-END"})
    equal(#collapsed,1)
...
local function passageVacaturControl(runtime,commitmentId,assemblyA,assemblyB,vehicleA,vehicleB)
    ...
    local function participant(id,vehicle)
        local grant=currentBoundedAuthorityGrant(runtime,id)
        local request=runtime.boundedAuthority:materializeRequest({boundedAuthorityId=grant.identity})
        drive[vehicle]=true; holds[vehicle]=true; configuration[vehicle]=true
        return {name=id,assemblyId=id,vehicle=vehicle,request=request}
    end
...
test("Cooperative Passage Leg: physical vacatur precedes leg BA and AU release",function()
    ...
    local boundaries={}
    local vacate=control.vacateParticipant
    control.vacateParticipant=function(self,...)
        equal(runtime.authorities:ownerOf("AS-A"),commitmentId)
    ...
    local releaseBA=runtime.boundedAuthority.releaseForCommitmentAssembly
    runtime.boundedAuthority.releaseForCommitmentAssembly=function(self,cid,id,...)
        equal(boundaries[1],"PHYSICAL"); boundaries[#boundaries+1]="BA"
        return releaseBA(self,cid,id,...)
    end
    local releaseAU=runtime.authorities.release
    runtime.authorities.release=function(self,token)
        if token.assemblyId=="AS-A" then equal(boundaries[2],"BA"); boundaries[#boundaries+1]="AU" end
        return releaseAU(self,token)
    end
    local result=OuttaMyWay.LiveTrafficCommitmentLifecycle.applyCooperativePassageParticipantLosses(runtime,{endedEpisodeIds={"JE-A"}},{identity="OBS-ORDER"})
...
test("Cooperative Passage Leg: rejected survivor permission fails held and retires stale BA",function()
    ...
    control.acceptSurvivorPermission=function() return false,"TEST_REBIND_REJECTED" end
    local result=OuttaMyWay.LiveTrafficCommitmentLifecycle.applyCooperativePassageParticipantLosses(runtime,{endedEpisodeIds={"JE-A"}},{identity="OBS-REJECT"})
...
test("Cooperative Passage Leg: physical neutralisation refusal retains leg authority",function()
    ...
    equal(control.run.phase,"FAILED_HELD"); equal(continued(),0)
    equal(runtime.authorities:ownerOf("AS-A"),commitmentId)
    equal(runtime.boundedAuthority:isCurrent(old.boundedAuthorityId),true)
    equal(#runtime.obligations:openForOwner(commitmentId),2)
end)
...
test("Cooperative Passage Leg: same sealed two-leg loss never transiently continues a doomed survivor",function()
    ...
    runtime.jobEpisodes.records["JE-A"]={identity="JE-A",assemblyId="AS-A",status="ENDED"}
    runtime.jobEpisodes.records["JE-B"]={identity="JE-B",assemblyId="AS-B",status="ENDED"}
    local outcomes=OuttaMyWay.LiveTrafficCommitmentLifecycle.applyCooperativePassageParticipantLosses(
        runtime,{endedEpisodeIds={"JE-A","JE-B"},observationSnapshotId="OBS-SEALED-BOTH"},{identity="OBS-SEALED-BOTH",assemblies={},playerControl={}})
    equal(#outcomes,2)
    equal(continued(),0)
    equal(control.run,nil)
    equal(runtime.commitments:get(commitmentId).state,"SUCCEEDED")
end)
...
test("Cooperative Passage Leg: former-participant hard safety distinguishes Job Episode end from physical removal",function()
    ...
    local survivor={
        vehicle=vehicleB,assemblyId="AS-B",
        transitPassageEnvelope=envelope
    }
    local run={
        participants={former,survivor},
        passageArrangement={nominalInterAssemblyClearanceM=1}
    }

    local supported,reason=control:_formerParticipantOccupancySupport(run,nil)
    equal(supported,false)
    equal(reason,"PASSAGE_SUPPORT_LOSS_FORMER_PARTICIPANT_CURRENT_OCCUPANCY:AS-A")

    former.vacaturEvidence={kind="POSITIVE_VEHICLE_RUNTIME_REMOVAL",positiveRemoval=true}
    supported,reason=control:_formerParticipantOccupancySupport(run,nil)
...
test("Cooperative Passage Leg: unexpected Bounded Authority loss during a live Passage Leg remains fail closed",function()
    ...
    control.run={mode="COOPERATIVE_PASSAGE_GUIDE",commitmentId="CM-BA-LOSS",phase="GUIDE_TEST",phaseStartedAt=0,startedAt=0,a=a,b=b,participants={a,b}}
    control:update(16)
...
test("Cooperative Passage Leg: transient unavailable raw Job token does not vacate a live Passage Leg",function()
    ...
    control.run={mode="COOPERATIVE_PASSAGE_GUIDE",commitmentId="CM-JOB-UNCERTAIN",phase="GUIDE_TEST",phaseStartedAt=0,startedAt=0,guideIndex=1,guide={gates={{index=1}}},
...
test("Cooperative Passage Leg: raw Job contradiction waits for semantic lifecycle instead of fail-held",function()
    ...
    local ok,err=pcall(function()
        ...
        control.run={
            mode="COOPERATIVE_PASSAGE_GUIDE",commitmentId="CM-SEMANTIC-PENDING",
...
test("Cooperative Passage Leg: vacatur before execution-origin rebase preserves survivor guide geometry",function()
    ...
    local control=OuttaMyWay.CooperativePassageControl.new(runtime,{holdMechanism={},driveMechanism={},configurationMechanism={}})
    control.run={
        mode="COOPERATIVE_PASSAGE_GUIDE",commitmentId="CM-REBASE-VACATED",subjectAssemblyId="AS-A",otherAssemblyId="AS-B",thirdPartyConstraints={},
...
test("Cooperative Passage: failed guide holds compact configuration without restore request",function()
    ...
    local control=OuttaMyWay.CooperativePassageControl.new({},donor)
    control.run={
        mode="COOPERATIVE_PASSAGE_GUIDE",commitmentId="CM-FAIL-PRESERVE",guide={identity="PG-FAIL-PRESERVE"},
...
test("Cooperative Passage: native blocked signal does not independently abort an active guide",function()
    ...
    local control=OuttaMyWay.CooperativePassageControl.new({},donor)
    control.run={
        mode="COOPERATIVE_PASSAGE_GUIDE",commitmentId="CM-BLOCKED-OBSERVATION",failureReason=nil,phase="GUIDE_TRAVERSAL",phaseStartedAt=0,guideIndex=1,
...
test("Cooperative Passage Leg: vacating active restoring participant starts survivor return without repeating completed debt",function()
    ...
    equal(beginReturn,0); equal(beginRestore,0)
    control:continueAfterParticipantVacatur("CM-VACATE-RESTORE","AS-A")
    equal(a.vacated,true)
    equal(beginReturn,0)
...
test("Cooperative Passage Leg: mixed vacated and handed-back final evidence does not claim same jobs or both restored",function()
    ...
    equal(evidence.kind,"COOPERATIVE_PASSAGE_LAST_LEG_DISSOLVED_AFTER_BASIS_CESSATION")
...
test("Job-Episode Dependency Collapse: ended Job Episode collapses dependent quiescent Cooperative Passage traffic Commitment before terminal succession",function()
    ...
    local dependent=runtime.commitments:create({
        ...
        governingBasis={responsibilityKey="cooperative-passage:REL-ENDED",dependentPairReferenceKey="pair:REL-ENDED",dependentJobEpisodeIds={"JE-END","JE-KEEP"}},
        situationDependencies={"SITUATION-1"}
    })
    local obligation=runtime.obligations:create({
        origin={kind="TRAFFIC_INTERVENTION"},basis={kind="ACTION_SPACE_REGULATION",conflictIdentity="REL-ENDED"},ownerCommitmentId=dependent.identity,
        requiredOutcome={kind="ACTION_SPACE_REGULATION_PRESERVED_UNTIL_RELATIONSHIP_MATURES_OR_DISSOLVES"},requiredAuthority={capabilities={"REGULATE_SPEED"}},
...
test("Phase13 direct Cooperative Passage substrate targeting is purpose and Job-Episode bound",function()
    ...
    local requirement="cooperative-passage:OC-DIRECT-TARGET"
    local retained={
