...
**Validated precursor:** v0.1.14.3 D-0198 Regulation Authority Semantics passed TS009/TS016b field observation; v0.1.14.4 D-0199 terminal courtesy validation was blocked before D-0147 admission by a retained quiescent D-0155 Commitment whose governing traffic Encounter lost a Job Episode at natural completion.

**Discovery — Job Episode Dependency Collapse:** Encounter termination and traffic Commitment termination were not coupled. A D-0146/D-0155 relationship may legitimately survive transient trajectory uncertainty, but cannot survive positive termination of a Job Episode on which its Encounter depends. Natural completion is basis cessation, not `SOURCE_INTENT_TERMINATION`.

**D-0200 decision:** after Job Episode admission records positive termination, and before Situation publishes the next Commitment context, any live traffic Commitment explicitly dependent on an affected Encounter settles its open obligations by `BASIS_CESSATION`, releases kernel/control authority, and terminalises successfully because its traffic basis has ceased. The Encounter then terminates through the existing `JOB_EPISODE_ENDED` path. This removes obsolete traffic ownership before D-0147 terminal candidate selection.

**Control cleanup:** D-0141, D-0155 and D-0123 dispatcher-local speed leases owned by the collapsed Commitment are cleared so kernel terminality cannot leave stale owner-tag actuation state.

**Explicit non-change:** D-0197/D-0198 Regulation semantics are unchanged; D-0199 Courtesy Budget Ownership and 60 m maximum centroid settlement are unchanged; no new stationary-obstacle braking/avoidance is added; Cooperative Passage, HUD and spatial-awareness overlay are untouched. The separate observation that an active worker can continue at speed toward a newly stationary completed assembly is parked for post-canonical investigation.

**Field gate:** repeat the terminal completion theatre. Confirm a pre-completion quiescent D-0155 Commitment logs `JOB_EPISODE_DEPENDENCY_COLLAPSE` at Job Episode end, disappears from Commitment context, and D-0147 then admits Courtesy 1. Continue through Continuation Renewal and, if a later different active worker is blocked by the same completed assembly, verify D-0199 Courtesy 2 and Double Courtesy exhaustion.

## v0.1.14.4 TEST — D-0199 Courtesy Budget Ownership + Bounded Centroid Settlement

**Validated precursor:** v0.1.14.3 D-0198 Regulation Authority Semantics passed TS009/TS016b field observation; ordinary behaviour was restored while strong D-0197 quiescence remained intact. No further Regulation change is included here.

**New terminal evidence:** after Courtesy 1, the completed Condor could later obstruct S416 but Courtesy 2 was not admitted because the renewal latch still required the later blocker to be the original authorising productive assembly. The courtesy budget itself belongs to the moved completed assembly, not to the first worker that happened to be blocked by it. Large-field Stage-1 centroid travel also needs a bounded maximum without recreating the historical Courtesy Distance Saturation defect.

**D-0199 decision:**
- **Courtesy Budget Belongs to the Obstacle:** the completed Job Episode owns the finite two-move budget. Courtesy 1 records its released/authorising worker(s) only as Continuation-Renewal witnesses. After those witnesses positively resume GIANTS-owned progression, any later active worker positively blocked by the same completed assembly may authorise Courtesy 2.
- **Bounded Centroid Settlement:** Courtesy 1 keeps the immutable centroid bearing but travels `min(distance-to-centroid, 60 m)`. The 60 m literal is a maximum movement allowance only, never a minimum prerequisite. Small fields still reach the centroid; larger fields stop after 60 m on the same fixed bearing.
- Courtesy 2 remains the single protected-away Final Boundary Settlement with containment/current-occupancy clearance; unsupported Stage 2 exhausts, no alternate-edge search exists, and no third automatic relocation is permitted.

**Explicit non-change:** D-0197/D-0198 Regulation behaviour, D-0146 Cooperative Passage, D-0192 Axis Return, Passage lateral selection, HUD/UI and the proposed Field World Spatial Awareness overlay are unchanged.

**Field gate:** rerun TS010 terminal completion (or an equivalent multi-worker terminal case). Confirm a small-field first courtesy reaches the centroid; confirm a later different productive worker blocked by the same completed assembly can authorise the second courtesy after Continuation Renewal; confirm no third move. A large-field observation is useful if available to confirm the 60 m cap, but is not required to re-prove small-field correctness.
...
**Field evidence:** v0.1.6.4 passed TS009, TS010S and TS015, but TS016 again remained in `CONFIGURING`. Patriot completed a real fold; the S416/K105 side retained `foldMoveDirection=1` without visible animation or configuration change. This disproves D-0178's assumption that non-zero `foldMoveDirection` reliably denotes physical fold motion. `isBlocked` remains secondary evidence only.

**D-0179:** Representation now establishes a Job-Start Physical Capability Record once per Job Episode: cached DISC/local geometry, a cached `TRANSIT_BASE` assembly footprint, and an OuttaMyWay semantic `isFoldable` plus active Transit actuator set. Foldability is derived only from the **selected runtime** `spec_foldable` configuration (`hasFoldingParts`, instantiated `foldingParts`, `allowUnfoldingByAI`, and callable fold actuation). Shop-available but unselected XML alternatives carry no authority; the stale `foldingConfiguration(0)...allowUnfoldingByAI` probe is removed.

Cooperative Passage no longer rediscovers fold capability. If cached `isFoldable=false`, configuration is immediate non-veto. If true, Control commands only cached active actuators and waits for each commanded actuator's requested `foldAnimTime` endpoint. Settlement is bounded by cached `maxFoldAnimDuration * 1.5 + 2000 ms`, capped at 35 s, with a 30 s fallback when duration is unavailable. **Transit Fold Settlement Exhaustion ends the configuration wait but does not assert successful compaction.** Passage then proceeds to Reality's existing physical/safety backstops.

The Transit footprint is now frozen on the first Job-Episode observation rather than rebuilt from changing member pose every passive sample. No Passage replan, Entry change, clearance change, D-0155 or D-0147 change is introduced.
...
**Field evidence:** v0.1.6.2 passed TS009, TS010S and TS015. TS016 failed in `CONFIGURING`: Patriot + S416 both reported accepted Transit requests (`changed=2`), Patriot physically compacted, while the S416/Bredal side did not acquire the universal `allFolded=true` condition. This repeats the earlier D-0170 finding that command acceptance and aggregate fold bookkeeping are not semantic Transit authority.

**Bench evidence before implementation:** TS015 Condor/Patriot representation does not expose a Transit-compatible current directional envelope during the roughly 15 s real fold. The current representation remains far larger/unavailable until physical compaction is realised, so the D-0176 5% geometry test does not release the sprayers prematurely.

**D-0177:** on the primary `TRANSIT_BASE` path, Control no longer reads `allFolded`, `allDeployed`, `transitionCount`, or `prepareCompact()` success/failure to decide Passage readiness. Transit is still always attempted. Readiness is determined only by current represented directional occupancy versus the cached Transit envelope using the existing 5% per-directional-extent similarity allowance. Representation may continue to use mechanical state internally to decide which current geometry is valid.

No Passage geometry, Entry, guide, clearance, speed, Execution-Origin Capture, D-0155 or D-0147 behaviour is changed. The legacy configuration-conditioned fallback remains isolated and unchanged.

# v0.1.6.2 TEST — D-0176 Transit Representation Similarity

Baseline: v0.1.6.1 TEST over owner-declared v0.1.6.0 canonical.

**Field evidence:** TS009 proved D-0175 compaction works: the S416 mower assembly reached Transit, but Passage then remained held on the MF/FW212 ignored-request branch. The rejecting trace reported `currentWidth=3.02` and `transitWidth=3.02`, showing exact directional containment was too brittle for representation/pose variation. TS010S completed successfully under v0.1.6.1.

**D-0176:** retain the cached Passage Transit envelope and admit current occupancy as Transit-realised when each directional extent is no more than 5% larger than the cached Transit extent. The tolerance is dimensionless representation similarity, not extra Passage clearance. No fresh Transit geometry is substituted at the gate, no Passage replan occurs, and accepted fold requests still wait for `allFolded=true`.

Primary validation remains TS009; TS010S is the first regression because it already passed the v0.1.6.1 gate.
...
# v0.1.2.1 TEST — Resolution-Space Progression Envelope

**Authority:** exact owner-declared canonical v0.1.2.0 (`1d861caca5f6d06656c0fcd41b1c278c01ac7cda223af1fdf101203fe6e0e583`; Git `f841717285fb9a02d4dc8c0c469adbecd56cb38c`; 316 files) is the implementation baseline.

**Authority-Layer Impact Review:** Representation already exposes pair separation and observed progression; Field World Membership, Operational Membership, Situation Relevance, Passage eligibility, Decision authority and Commitment purpose remain unchanged. The review discovered **Magnitude Leakage**: the inherited 8 km/h value was chosen by Situation Assessment and could suppress a candidate when native progression happened to be <=8 km/h. v0.1.2.1 removes Control magnitude from Situation/Candidate evidence; Control owns the envelope.

**D-0155 implementation:** adds `ResolutionSpaceProgressionEnvelope.lua`. At admission it withholds a provisional **75% Resolution Contingency Reserve**, uses the remaining 25% as ordinary progression allowance, seeds the zero-terminal-progression policy curve from current native progression, floors each resulting cap to a whole integer km/h, and expresses Hold as the zero endpoint of that same curve. No physical braking/deceleration model is introduced.

**Reverse-created reserve:** current physical separation may increase while the protected/uncertain participant reverses, but conservative authorised distance cannot increase within the same unresolved Situation. The difference is recorded as **Reverse-Created Resolution Reserve** and is not immediately spendable ordinary progression authority.

**Role migration:** **Magnitude Rebase on Role Migration** preserves the existing absolute contingency reserve and consumed allowance, samples the newly regulated participant's current progression, and reconstructs the policy trajectory only over remaining ordinary allowance. The same Commitment and obligation are retained.

**Retired D-0146 Control behaviour:** fixed generic 8 km/h magnitude; candidate suppression based on already being <=8 km/h; retrospective `cap realised + still closing => Hold`; transient non-closing as an automatic return to 8 km/h. Situation still owns positive release/supersession.

**Explicitly unchanged:** 80 m locality ceiling; Pair-Specific/Configuration-First Cooperative Passage mechanics; Pre-Productive Intent Relevance; Safe Release; Recovery; D-0147; other parked literal work.

**Bench validation before field test:** 254/254 Lua behavioural tests PASS; 96/96 Python structural tests PASS; 25/25 RRS tests PASS. New behavioural coverage proves prospective tightening, zero-cap Hold, reverse-created reserve protection, low-speed admission and role-migration rebase. Live TS010 then TS015/TS016 remain required.
...
# v4.7.120 TEST BUILD — D-0147 Exit Alignment continuation

Owner-declared v4.7.112 remains canonical. v4.7.120 is a narrow production E&OE correction over the live-supported v4.7.119 Vehicle Activity Context.

- v4.7.119 validated post-job steering realisation: `forceIsActive=true` produced `isActive=true`, non-zero wheel steering angles and realised yaw without restarting a GIANTS Job Episode.
- TS016 v4.7.119 nevertheless showed **Exit Vector / Exit Heading Mismatch**: Patriot pursued the fixed oblique endpoint through an approximately 86-degree arc instead of settling near the intended heading/outward bisector, then stopped before its represented compact footprint was wholly outside.
- Candidate now supplies one deterministic Exit Alignment direction and no terminal destination/clearance target.
- `TerminalEgressControl` holds that fixed world direction until Positive Field-Exit Settlement; there is no `TARGET_REACHED_WITHOUT_POSITIVE_FIELD_EXIT` path and no target extension.
- `PostJobActuationAuthority` uses GIANTS `AIVehicleUtil.driveInDirection()` as the direction-holding actuator. The AutoDrive-observed legacy `motor` / `cruiseControl` compatibility fields are supplied only during the helper call and restored immediately.
- Vehicle Activity Context, 8 km/h, Terminal Resolution Commitment, Player Claim, source-reactivation pre-emption, Actuation Neutralisation, one selected boundary and one-manoeuvre watchdog remain unchanged.
- No alternate heading, second boundary, route search, reverse rescue, retry, post-exit parking or synthetic AI job is introduced.
...
## v4.7.43 TEST BUILD — Initial Head-On Live Decision Vertical Slice

- begins from owner-declared canonical v4.7.41 (`a15b5f3534545d4cbefd1cfc291f254d0921472dc35c09f900935e9f59cddb15`; Git `1eb64d6fcf328fb566b4aa27d83fe5fdb7ab2911`; 269 files); this is an implementation test build, not a certification candidate;
- carries forward the v4.7.42 implementation experiment that put canonical D-0115 Traffic Policeman ordering into the real `DecisionSelector`; live testing correctly showed no vehicle-behaviour change because live Candidate support still exposed only `CONTINUE_OBSERVATION`;
- adds a deliberately narrow live vertical slice for the initial pure established head-on so the real Candidate → Constraint → Decision path can reach `REPOSITION` before the P22 TS015 actuator is dispatched;
- requires explicit `otmP22 resolveheadon <Condor|Patriot>` operator arming. The named participant is pinned only as the TS015 fixture participant to be relocated; the pin has no production movement-priority authority and does not invent a Productive/Productive tie-break rule;
- requires the same two GIANTS Job Episodes, same active Encounter, positive field-bounded Future-Space intersection, both Local Intents `SETTLED_CONTINUATION`, and no already-positive Current Space/plan-view physical interaction before the test Candidate can be published;
- supplies same-Operational-Picture D-0115 exhaustion evidence for Observe, Regulation and in-path Hold: the explicit fixture assertion makes further Observe non-directive for this test, creep cannot resolve pure head-on spatial incompatibility, and an in-path Hold would create Static Obstacle Conversion;
- keeps production role selection, Refuge Region qualification, obstacle-clearance authority, Safe Release authority, Vulnerable Space/Convergent Projection construction and production Control explicitly unresolved/disabled;
- dispatches the already-validated P22 TS015 Hold/compact/egress/refuge/rejoin/restore/handoff mechanism only after the real live Decision selects `REPOSITION`; the existing fixture continues to fail closed at its own preconditions;
- leaves existing HUD remnants enabled so this behavioural slice does not mix presentation cleanup with traffic logic;
- offline validation is necessary but not certification: live GIANTS validation by the owner is required before any RRS/canonicalisation step.
...
- Archives the exact 48-file v4.6.78 active script tree under `scripts/archive/v4_6_78/`; active code cannot import it.
- Replaces the active loader and configuration with a minimal inert bootstrap.
- Adds immutable canonical record contracts, identity and epoch primitives, the three-state Commitment machine, Commitment Registry, Obligation Ledger, Authority Registry and Effective Actuation Composition validation.
- Adds deterministic Lua conformance tests and repository structural tests.
- Adds ADR-0020, the accepted Core Replacement Implementation Contract and legacy module disposition records.
- Intentionally removes all gameplay observation and intervention. v4.7.0 has zero Control authority.
