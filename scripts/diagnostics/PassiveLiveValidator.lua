...
function Validator:_logRecordDiagnostics(record,due,nowMilliseconds)
    for _,item in OuttaMyWay.ValueRecord.ipairs(record.assemblyDiagnostics or {}) do
        ...
        if representation.configurationProfileId~=nil and self.profileDiagnosticSignatures[item.assemblyReferenceKey]~=profileSignature then
            ...
            logInfo(string.format("PROFILE assembly=%s profile=%s selector=%s inventory=%d participating=%d inactive=%d unresolved=%d runtimeConfirmed=%d donorFallback=%d activeNodes=%s inactiveNodes=%s unresolvedNodes=%s",tostring(item.assemblyReferenceKey),tostring(representation.configurationProfileId),tostring(representation.configurationSelectorSummary or "n/a"),tonumber(representation.inventoryPrimitiveCount) or 0,tonumber(representation.participatingPrimitiveCount) or 0,tonumber(representation.inactivePrimitiveCount) or 0,tonumber(representation.unresolvedPrimitiveCount) or 0,tonumber(representation.runtimeConfirmedPrimitiveCount) or 0,tonumber(representation.donorFallbackPrimitiveCount) or 0,table.concat(representation.participatingPrimitiveNames or {},","),table.concat(representation.inactivePrimitiveNames or {},","),table.concat(representation.unresolvedPrimitiveNames or {},",")))
        end
    end
