...
local publication=OuttaMyWay.LogPublication.origin("PASSIVE_LIVE_VALIDATOR")
...
local function logWarning(code,formatText,...)
    return publication:warning("DIAGNOSTIC",code,formatText,...)
end
local function publicationEnabled()
    local eligible=publication:isEligible("DIAGNOSTIC","INFO","PASSIVE_RUNTIME_SNAPSHOT")
...
function Validator:_warn(code,details,nowMilliseconds)
    ...
    if last==nil or nowMilliseconds-last>=heartbeat then
        ...
        logWarning(tostring(code),"subject=%s operation=%s reason=%s outcome=%s",tostring(pair or "global"),tostring(details and details.operationId or "n/a"),tostring(details and (details.reason or details.exclusionReason) or "n/a"),tostring(details and details.principalOutcome or "n/a"))
    end
end
...
function Validator:_logCycleDiagnostics(cycle,due,nowMilliseconds)
    for _,item in ipairs(cycle.assemblyAcquisition or {}) do
        ...
        if self.acquisitionSignatures[item.assemblyReferenceKey]~=signature or due then
            ...
            logInfo("PASSIVE_ACQUISITION","ACQUISITION assembly=%s name=%s activeJobMember=%s poseResolved=%s node=%s nodeSource=%s reason=%s position=(%s,%s) heading=(%s,%s)",tostring(item.assemblyReferenceKey),tostring(item.name),booleanText(item.activeJobVehicleMembership),booleanText(item.poseResolved),tostring(item.node or "n/a"),tostring(item.nodeSource or "n/a"),tostring(item.poseReason or "n/a"),numberText(item.x),numberText(item.z),numberText(item.headingX),numberText(item.headingZ))
        end
    end
...
function Validator:_logFutureSpaceRelationships(projection)
    for _,relationship in OuttaMyWay.ValueRecord.ipairs(projection.futureSpaceRelationships or {}) do
        ...
        if self.futureSpaceLogSignatures[key]~=signature then
            ...
            logInfo("PASSIVE_FUTURE_SPACE","FUTURE_SPACE pair=%s classification=%s outcome=%s subjectIntent=%s/%s otherIntent=%s/%s subjectBoundary=%s otherBoundary=%s positive=%s unresolved=%s authority=%s control=false",
...
function Validator:_logTrajectoryConflictKnowledge(picture,due)
    for _,item in OuttaMyWay.ValueRecord.ipairs(picture and picture.trajectoryKnowledge or {}) do
        ...
        if self.trajectoryLogSignatures[key]~=signature or due then
            ...
            logInfo("PASSIVE_TRAJECTORY","TRAJECTORY assembly=%s job=%s status=%s transition=%s establishedDir=(%s,%s) anchor=(%s,%s) currentDir=(%s,%s) currentDot=%s forming=%sm aligned=%sm excursion=%s excursionDistance=%sm context=%s productivePositive=%s authority=TRAJECTORY_CONFLICT_ASSESSMENT diagnosticOnly=true",
    ...
    for _,item in OuttaMyWay.ValueRecord.ipairs(picture and picture.opposedCorridorKnowledge or {}) do
        ...
        if self.opposedCorridorLogSignatures[key]~=signature or due then
            ...
            logInfo("PASSIVE_OPPOSED_CORRIDOR","OPPOSED_CORRIDOR pair=%s operation=%s classification=%s status=%s reason=%s participation=%s/%s pending=%s/%s passageEligible=%s trajectoryDot=%s mutuallyFacing=%s overlap=%s positiveOverlap=%s overlapM=%s currentDot=%s closingRate=%s currentOpposed=%s closingPositive=%s stable=%s/%s excursions=%s/%s settled=%s/%s relationship=%s relationshipReason=%s primitives=%d+%d actionSpace=%s actionReason=%s regulated=%s protected=%s excursion=%s role=%s actionSep=%s actionOverlap=%s actionNative=%s magnitudeAuthority=BOUNDED_AUTHORITY authority=TRAJECTORY_CONFLICT_ASSESSMENT diagnosticOnly=true",
                ...
                numberText(action.separationM),numberText(actionOverlap.overlapM),numberText(action.nativeUnrestrictedKmh))
        end
    end
end
