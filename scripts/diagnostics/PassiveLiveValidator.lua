...
function Validator.new(runtime,source,support,probe,fieldWorldSnapshots)
    return setmetatable({
        ...
        acquisitionSignatures={},assemblyDiagnosticSignatures={},warningLastAt={},
...
function Validator:loadMap()
    ...
    self.acquisitionSignatures={}; self.assemblyDiagnosticSignatures={}; self.warningLastAt={}; self.previousEncounters={}; self.previousPairs={}; self.lastEncounterHeartbeatAt=-math.huge
    ...
    logInfo("Field World Equivalence Authority and bounded interaction diagnostics active; predicates unchanged; unresolved identity grants no Operation admission; replacement Control authority disabled")
end
...
function Validator:_logRecordDiagnostics(record,due,nowMilliseconds)
    for _,item in OuttaMyWay.ValueRecord.ipairs(record.assemblyDiagnostics or {}) do
        ...
        if self.assemblyDiagnosticSignatures[item.assemblyReferenceKey]~=signature or due then
            ...
            logInfo(string.format("ASSEMBLY assembly=%s name=%s operation=%s activeJobMember=%s pose=%s nodeSource=%s poseReason=%s width=%s length=%s radius=%s components=%d representation=%s coverageComplete=%s conservative=%s underApproximationRisk=%s motion=%s reportedSpeed=%s derivedSpeed=%s headingTravelDot=%s yawRateDegPerSec=%s blocked=%s",tostring(item.assemblyReferenceKey),tostring(item.name),tostring(item.operationId or "n/a"),booleanText(item.activeJobVehicleMembership),booleanText(item.poseResolved),tostring(item.nodeSource or "n/a"),tostring(item.poseReason or "n/a"),numberText(item.width),numberText(item.length),numberText(item.radius),tonumber(item.componentCount) or 0,tostring(item.representationFitnessState or "n/a"),booleanText(item.coverageComplete),booleanText(item.conservative),booleanText(item.underApproximationRisk),tostring(motion.classification or "n/a"),numberText(motion.reportedSpeedMps),numberText(motion.positionDerivedSpeedMps),numberText(motion.headingToTravelDot),numberText(motion.yawRateDegreesPerSecond),booleanText(item.blocked)))
        end
    end
