...
local function relocationSerializationBeneficiaries(record,context,snapshot)
    local ids={}
...
local function physicalSpec(picture,record,phase,objective,objectiveReason,protected)
    ...
    constraints.CONTROL_CAPABILITY_AVAILABILITY=packet("ObstructionRelocationControl reuses supported compaction, Vehicle Activity Context and forward-only driveInDirection actuation; the existing Regulation lease substrate provides a zero-speed Relocation Serialization hold for the authorising productive assembly/assemblies during translation",{controlModule="ObstructionRelocationControl",postJobActuation=true,relocationSerialization=true,holdActuator="REGULATE_SPEED_0",moveForwards=true,playerClaimWitness="vehicle:getIsEntered()"})
    constraints.CONTINUING_INTENT_PRIORITY=packet("The completed assembly moves while the active assembly/assemblies whose conflict authorised D-0147 retain their GIANTS jobs but are temporarily held for the translational Relocation Serialization Interval",{terminalAssemblyId=record.assemblyId,activeDemandAssemblyIds=record.obstructedDemandAssemblyIds,relocationSerializationBeneficiaries=protected,productiveJobsRemainGiantsOwned=true})
...
function Support:buildProjectedGroup(picture,snapshot,projection,targetPictureId,targetEpoch)
    ...
    elseif record.exhausted==true then specification=settlementSpec(record,"OBJECTIVE_FAILED")
        ...
        local phase="COMPACT"
        local protected=relocationSerializationBeneficiaries(record,nil,snapshot)
    ...
    return {
        supportBoundary={mode="D0147_BOUNDED_TERMINAL_EGRESS",supportedCandidateClasses={specification.capability},physicalCapabilitiesImplemented=true,controlAuthority="D0147_POST_JOB_PLUS_RELOCATION_SERIALIZATION",boundedScope="ONE_COMPLETED_ASSEMBLY_MAX_TWO_FIXED_DIRECTION_COURTESY_MOVES_INTERIOR_THEN_PROTECTED_AWAY_BOUNDARY_WITH_CURRENT_OCCUPANCY_CLEARANCE",automaticTerminalEgress=true},
...
function Support:attach(picture,snapshot)
    ...
    elseif record.exhausted==true then specification=settlementSpec(record,"OBJECTIVE_FAILED")
        ...
        local protected=relocationSerializationBeneficiaries(record,context,snapshot)
        local objective,objectiveReason=nil,nil
