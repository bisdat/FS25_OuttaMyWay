...
local function activityCorroboration(object)
    ...
    local aiActiveObserved = false
    ok, value = safeCall(object, "getIsAIActive")
    if ok then
        aiActiveObserved = true
        if value == true then aiActive = true end
    end
    return fieldActive, aiActive, hasFieldWorker, aiActiveObserved
end
    ...
    for _, key in OuttaMyWay.ValueRecord.ipairs(keys) do
        ...
        local raw = {
            ...
            fieldWorld = {
                ...
                canonicalRootRing=representative and representative.canonicalRootRing or nil,
                canonicalRootVertices=representative and representative.canonicalRootVertices or nil,
                canonicalIslandRings=representative and representative.canonicalIslandRings or {},
        ...
        for _, worker in OuttaMyWay.ValueRecord.ipairs(group.workers) do
            ...
            if worker.pose ~= nil then
                ...
                raw.physicalRepresentationEvidence[#raw.physicalRepresentationEvidence + 1] = {
                    ...
                    question = "CURRENT_CONFLICT_SUPPORT", assessmentHorizon = 0, structurallyValid = worker.radius ~= nil,
        ...
        for i = 1, #group.workers - 1 do
            for j = i + 1, #group.workers do
                ...
                if eligible then
                    ...
                    -- Encounter admission authority is positive Current Space or the canonical
                    -- field-bounded Future Space relationship. Superseded fixed-horizon future
                    -- prediction has been removed from active runtime and diagnostics.
                    local currentPositive=observed.current==true or currentFootprint.current==true
                    ...
                    if admissionPositive then
                        ...
                        raw.geometry.interactionEvidence[#raw.geometry.interactionEvidence + 1] = {
                            ...
                            provenance = {
                                ...
                                currentSpace = {scalar=observed.current==true,filteredPlanView=currentFootprint.current==true}
                            }
                        }
                    end
                    if observed.headingDot > 0.5 and observed.closingRate > 0.05 then
                        ...
                        raw.motion.closureEvidence[#raw.motion.closureEvidence + 1] = {
                            ...
                            closingObserved = true, closingRate = observed.closingRate,
