...
function Runtime:initialize()
    ...
    print(string.format("FS25_OuttaMyWay v%s CANONICAL CANDIDATE loaded; D-0186 Regulation-Hold Boundary ACTIVE: positive Regulation preserves native drive permission, while an effective 0.0 km/h cap revokes drive permission so GIANTS never receives allowed=true,maxSpeed=0; D-0184 Closure C, D-0183 cached-actuator restoration, D-0181 D-0146-only fail-closed Passage, D-0179 Job-Start Physical Capability Record, D-0155 Resolution-Space policy and D-0147 Bounded Infield Retreat are otherwise unchanged; automatic terminal-yield test gate=%s; Player Claim uses vehicle:getIsEntered()",OuttaMyWay.VERSION,tostring(OuttaMyWay.AUTOMATIC_TERMINAL_EGRESS==true)))
end
