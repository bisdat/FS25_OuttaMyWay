...
function Runtime:initialize()
    ...
    print(string.format("FS25_OuttaMyWay v%s CANONICAL CANDIDATE loaded; D-0184 Legacy Authority Closure C ACTIVE: retired D-0143/TS015 fixture implementation, committed-transition/guarded-recovery test bridges, dead generic Passage restore and the in-tree v4.6.78 script archive are removed; D-0123 Native Handover Creep remains 1.0 km/h under production naming because HOLD prevented GIANTS restart into handover; D-0183 cached-actuator restoration, D-0181 D-0146-only fail-closed Passage, D-0179 Job-Start Physical Capability Record, D-0155 Resolution-Space policy and D-0147 Bounded Infield Retreat are otherwise unchanged; automatic terminal-yield test gate=%s; Player Claim uses vehicle:getIsEntered()",OuttaMyWay.VERSION,tostring(OuttaMyWay.AUTOMATIC_TERMINAL_EGRESS==true)))
end
