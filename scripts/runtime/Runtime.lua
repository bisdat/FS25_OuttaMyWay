...
function Runtime:initialize()
    ...
    print(string.format("FS25_OuttaMyWay v%s CANONICAL CANDIDATE loaded over owner-declared canonical v0.1.0.0; D-0146 Step-1 Situation Knowledge is live-validated and Step-2 Established Conflict -> Candidate-owned Local Passage Search -> Passage Guide -> Commitment/Control is ACTIVE; general production Control authority disabled outside bounded admitted commitments; Control authority disabled outside bounded admitted commitments; D-0147 governing architecture is Terminal Yield while Pending Player Reclamation, while v4.7.126 TEST implements one protected fixed-bearing 60 m Bounded Infield Retreat toward the Field World centroid at native maximum forward speed with no continuous course corrections; automatic test gate=%s; Player Claim uses vehicle:getIsEntered()",OuttaMyWay.VERSION,tostring(OuttaMyWay.AUTOMATIC_TERMINAL_EGRESS==true)))
end
