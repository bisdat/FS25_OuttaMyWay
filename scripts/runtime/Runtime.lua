...
function Runtime:initialize()
    ...
    print(string.format("FS25_OuttaMyWay v%s CANONICAL CANDIDATE loaded; D-0183 Cached-Actuator Restoration Symmetry ACTIVE: D-0146 restores only cached Transit actuators that physically changed, waits on their requested return endpoints with bounded settlement, and never uses aggregate allDeployed or live fold rediscovery for restoration; D-0181 Legacy Authority Closure A remains ACTIVE: D-0146 Cooperative Passage fails closed without cached TRANSIT_BASE geometry; D-0143 runtime Candidate/Control authority is retired; D-0179 Job-Start Physical Capability Record remains ACTIVE: Physical Assembly membership, DISC/local geometry, cached TRANSIT_BASE footprint and runtime-selected AI-reachable fold capability are established once per Job Episode; Passage performs no fold-capability rediscovery; TRANSIT_BASE always attempts only cached active Transit actuators; actuator endpoint settlement is bounded by cached runtime maxFoldAnimDuration plus policy margin (fallback ceiling when unavailable); settlement exhaustion removes configuration veto without asserting compaction; shop-available but unselected folding configurations, allFolded/allDeployed aggregate semantics, foldMoveDirection and current-vs-Transit geometry carry no TRANSIT_BASE settlement authority; Execution-Origin Capture, guide geometry, 0.95 m Crossing-Window policy floor, D-0155 Resolution-Space policy, D-0147 Bounded Infield Retreat and inherited 8 km/h Passage speed are otherwise unchanged; automatic terminal-yield test gate=%s; Player Claim uses vehicle:getIsEntered()",OuttaMyWay.VERSION,tostring(OuttaMyWay.AUTOMATIC_TERMINAL_EGRESS==true)))
end
