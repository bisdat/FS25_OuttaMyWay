...
function Runtime:initialize()
    ...
    print(string.format("FS25_OuttaMyWay v%s CANONICAL CANDIDATE loaded; D-0188 Termination Evidence Collapse ACTIVE: authoritative GIANTS source-job end evidence now ends the Job Episode directly with no SOURCE_INTENT_TERMINATION terminal cause or active-job takeover subtype taxonomy; RESTARTED/REPLACED episode succession remains explicit; D-0147 Terminal Occupancy still accepts only naturally ended non-succession episodes and Player Claim semantics are unchanged; D-0186 Regulation-Hold Boundary remains active; automatic terminal-yield test gate=%s",OuttaMyWay.VERSION,tostring(OuttaMyWay.AUTOMATIC_TERMINAL_EGRESS==true)))
end
