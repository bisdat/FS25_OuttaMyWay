...
function Runtime:initialize()
    ...
    self.trace:append("PROGRESSIVE_SITUATIONAL_SUFFICIENCY_INITIALIZED",self.epochs:next(),"geometryBoundedRelocation=true;obstructionRelocationCoreCapability=true;trajectorySituationKnowledge=true;trajectoryPersistence=true;opposedCorridorClassification=true;cooperativePassageOperationAware=true;followerBoundaryRegulation=true;turningRankAwareness=true;runtimeOwnedCycle=true;situationOwnsCurrentKnowledge=true;diagnosticsAuthority=false;typedBoundedControl=true")
...
function Runtime:_terminateActionSpaceRegulation(picture,evaluated,current,assessment)
    ...
    if corner and assessment.terminationEvidenceKind~="CORNER_COMPETING_DEMAND_POSITIVE_DISSOLUTION" then
        return {
            status="NO_DISPATCH",
            reason="CORNER_RIGHT_OF_WAY_TERMINATION_EVIDENCE_REQUIRED",
            detail=assessment.reason,
            actionSpaceRegulation=true,
            cornerRightOfWay=true,
            commitmentId=commitmentId
        }
    end
