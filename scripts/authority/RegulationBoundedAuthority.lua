...
function Authority:_releaseRelocationSerialization(commitmentId,reason)
    ...
    logInfo("DEBUG","RELOCATION_SERIALIZATION_RELEASE","commitment=%s released=%d reason=%s",tostring(commitmentId),released,tostring(reason))
    return released
end
...
function Authority:_applyRelocationSerialization(picture,evaluated,candidate,commitment,currentResponsibility,bridge)
    ...
    for _,item in OuttaMyWay.ValueRecord.ipairs(protected) do
        ...
        logInfo("DEBUG","RELOCATION_SERIALIZATION_APPLIED","commitment=%s assembly=%s ref=%s request=%s cap=0.00kmh",tostring(commitment.identity),tostring(item.assemblyId),tostring(item.referenceKey),tostring(request.identity))
    end
...
local function actionSpaceRegulationRelation(picture,lease)
    ...
    for _,knowledge in OuttaMyWay.ValueRecord.ipairs(picture.spatialConstraintKnowledge or {}) do
        ...
        for _,situation in OuttaMyWay.ValueRecord.ipairs(corner.sharedCornerSituations or {}) do
            if situation.identity==lease.conflictIdentity then return situation end
        end
    end
    return nil
end
...
-- Category-1 Corner incumbency is current Situation meaning that the assembly
...
currentCornerIncumbency=function(picture,assemblyId)
    ...
    for _,knowledge in OuttaMyWay.ValueRecord.ipairs(picture.spatialConstraintKnowledge or {}) do
        local corner=knowledge.cornerKnowledge or {}
        for _,engagement in OuttaMyWay.ValueRecord.ipairs(corner.engagements or {}) do
            if engagement.assemblyId==assemblyId and engagement.cornerIncumbent==true then return engagement end
        end
    end
    return nil
end
...
function Authority:_quiesceFollowerBoundaryActuation(picture,evaluated,candidate,bridge)
    ...
    logInfo("DEBUG","FOLLOWER_BOUNDARY_ACTUATION_QUIESCENT","commitment=%s pair=%s follower=%s reason=%s purposeRetained=true quiescenceCount=%d",tostring(lease.commitmentId),tostring(lease.pairKey),tostring(lease.followerAssemblyId),tostring(lease.quiescenceReason),tonumber(lease.quiescenceCount) or 0)
...
function Authority:neutralizeActionSpaceRegulationPhysical(picture,evaluated,reason)
    ...
    logInfo("DEBUG","ACTION_SPACE_REGULATION_RELEASE","commitment=%s conflict=%s regulated=%s ref=%s reason=%s",tostring(lease.commitmentId),tostring(lease.conflictIdentity),tostring(lease.regulatedAssemblyId),tostring(lease.regulatedReferenceKey),tostring(reason))
...
local function actionSpaceRegulationQuiescenceSupported(picture,lease,action)
    ...
    if marginDemand~=nil then
        ...
        logInfo("DEBUG","ACTION_SPACE_REGULATION_QUIESCENCE_VETO","commitment=%s conflict=%s regulated=%s protected=%s demand=%s claimClass=%s reason=POSITIVE_PAIR_LOCAL_RESOLUTION_MARGIN_DEMAND",
...
function Authority:_quiesceActionSpaceRegulationActuation(picture,evaluated,lease,relation,action)
    ...
    logInfo("DEBUG","ACTION_SPACE_REGULATION_ACTUATION_QUIESCENT","commitment=%s conflict=%s regulated=%s protected=%s actionSpace=NOT_REQUIRED actionReason=%s relationshipRetained=true quiescenceCount=%d",
...
function Authority:_continueActionSpaceRegulationReactivation(picture,evaluated,candidate,lease,bridge,applied)
    ...
    self.dispatchCount=self.dispatchCount+1
    local outcome=self:_outcome(request,"ACCEPTED",{kind="ACTION_SPACE_REGULATION_ACTUATION_REACTIVATED",capability="REGULATE_SPEED",effectClass=fixed and "INTENT_REVELATION_CREEP" or envelope.effectClass,maxSpeedKmh=cap},nil)
    logInfo("DEBUG","ACTION_SPACE_REGULATION_ACTUATION_REACTIVATED","commitment=%s conflict=%s regulated=%s protected=%s cap=%dkmh actionSpace=REGULATE_SUPPORTED envelopeRebased=true reactivationCount=%d",
...
function Authority:_updateActionSpaceRegulationEnvelope(picture,evaluated,candidate,lease,relation,relationshipReason)
    ...
    logInfo("DIAGNOSTIC","RESOLUTION_SPACE_PROGRESSION_ENVELOPE_UPDATE","commitment=%s conflict=%s regulated=%s protected=%s priorCap=%dkmh cap=%dkmh raw=%.2fkmh physical=%.2fm conservative=%.2fm reverseReserve=%.2fm contingency=%.2fm ordinaryRemaining=%.2fm effect=%s",
    ...
    if envelope.effectClass=="INTENT_REVELATION_CREEP" and priorCap~=requestedCap then
        logInfo("DIAGNOSTIC","INTENT_REVELATION_CREEP","commitment=%s conflict=%s regulated=%s protected=%s cap=%dkmh physical=%.2fm conservative=%.2fm contingency=%.2fm purpose=%s",
...
function Authority:_continueActionSpaceRegulationRoleMigration(picture,evaluated,candidate,lease,bridge,applied)
    ...
    if fixed then
        ...
        rebased,rebaseReason=OuttaMyWay.ResolutionSpaceProgressionEnvelope.rebaseRole(envelopeCopy,bridge.nativeUnrestrictedKmh,bridge.separationM)
        if rebased==nil or rebaseReason~=nil then
            if applied.authorityAcquired then
                OuttaMyWay.LiveTrafficCommitmentLifecycle.releaseSupportingRegulationAuthority(
                    self.runtime,applied.commitment.identity,bridge.regulatedAssemblyId,
                    {reason="ACTION_SPACE_REGULATION_ROLE_REBASE_FAILED:"..tostring(rebaseReason),
                     preserveAuthority=self:_otherRegulationPurposeOwnsAuthority(applied.commitment.identity,bridge.regulatedAssemblyId,"ACTION_SPACE_REGULATION")})
            end
    ...
    if started~=true then
        ...
        if applied.authorityAcquired then
            OuttaMyWay.LiveTrafficCommitmentLifecycle.releaseSupportingRegulationAuthority(
                self.runtime,applied.commitment.identity,bridge.regulatedAssemblyId,
                {reason="ACTION_SPACE_REGULATION_ROLE_MIGRATION_NEW_CONTROL_REQUEST_REJECTED:"..tostring(newResult),
                 preserveAuthority=self:_otherRegulationPurposeOwnsAuthority(applied.commitment.identity,bridge.regulatedAssemblyId,"ACTION_SPACE_REGULATION")})
        end
    ...
    local oldOwnerTag=lease.ownerTag or ACTION_SPACE_REGULATION_OWNER_TAG
    if oldToken~=nil then
        local syntheticCandidate={preconditions={},invalidationConditions={}}
        oldRequest=self:_regulationRequest(
            picture,evaluated,syntheticCandidate,applied.commitment,oldToken,
            {regulatedAssemblyId=lease.regulatedAssemblyId,regulatedReferenceKey=lease.regulatedReferenceKey,governingPurpose=lease.governingPurpose},
            "RELEASE",oldOwnerTag,nil,nil,lease.boundedAuthorityId)
        if oldRequest==nil then
            return {status="MAINTAINED",reason="ACTION_SPACE_REGULATION_ROLE_MIGRATION_OLD_BOUNDED_AUTHORITY_UNAVAILABLE",actionSpaceRegulation=true,commitmentId=lease.commitmentId}
        end
        local oldReleased=self.runtime.liveControlDispatcher:dispatch(oldRequest,nil)
        if oldReleased~=true and type(self.regulationControl.clearRegulationLeaseByReference)=="function" then
            self.regulationControl:clearRegulationLeaseByReference(lease.regulatedReferenceKey,oldOwnerTag)
        end
    elseif type(self.regulationControl.clearRegulationLeaseByReference)=="function" then
        self.regulationControl:clearRegulationLeaseByReference(lease.regulatedReferenceKey,oldOwnerTag)
    end
    ...
    lease.ownerTag=newOwnerTag
    lease.authorityTokenId=newToken.identity
    lease.boundedAuthorityId=newRequest.boundedAuthorityId
    ...
    if fixedCorner then
        logInfo("DEBUG","CORNER_RIGHT_OF_WAY_ROLE_MIGRATED","commitment=%s situation=%s oldRegulated=%s newRegulated=%s oldProtected=%s newProtected=%s cap=%dkmh",
            tostring(lease.commitmentId),tostring(lease.conflictIdentity),tostring(oldRegulatedAssemblyId),
            tostring(lease.regulatedAssemblyId),tostring(oldProtectedAssemblyId),tostring(lease.protectedAssemblyId),newCap)
        return {status="ROLE_MIGRATED",reason="CORNER_RIGHT_OF_WAY_CURRENT_DECISION_REASSIGNED_TEMPORARY_PRIORITY",
            request=newRequest,releaseRequest=oldRequest,outcome=outcome,actionSpaceRegulation=true,cornerRightOfWay=true,commitmentId=lease.commitmentId}
    end
    logInfo("DIAGNOSTIC","ACTION_SPACE_REGULATION_ROLE_REBASE","commitment=%s conflict=%s oldRegulated=%s newRegulated=%s oldProtected=%s newProtected=%s cap=%dkmh ordinaryRemaining=%.2fm contingency=%.2fm reverseReserve=%.2fm rebaseCount=%d",
        tostring(lease.commitmentId),tostring(lease.conflictIdentity),tostring(oldRegulatedAssemblyId),
        tostring(lease.regulatedAssemblyId),tostring(oldProtectedAssemblyId),tostring(lease.protectedAssemblyId),newCap,
        tonumber(rebased.remainingOrdinaryM) or 0,tonumber(rebased.contingencyReserveM) or 0,
        tonumber(rebased.reverseCreatedReserveM) or 0,tonumber(rebased.roleRebaseCount) or 0)
    return {status="ROLE_MIGRATED",reason="ACTION_SPACE_REGULATION_CURRENT_SITUATION_REASSIGNED_ROLES_WITH_MAGNITUDE_REBASE",
        request=newRequest,releaseRequest=oldRequest,outcome=outcome,actionSpaceRegulation=true,commitmentId=lease.commitmentId}
end
...
function Authority:_continueActionSpaceRegulationInitial(picture,evaluated,candidate,bridge,applied)
    ...
    if fixedCornerRightOfWay then
        logInfo("DEBUG","CORNER_RIGHT_OF_WAY_REGULATION_APPLIED","commitment=%s situation=%s corner=%s regulated=%s protected=%s cap=1kmh purpose=%s",
            tostring(applied.commitment.identity),tostring(bridge.conflictIdentity),tostring(bridge.cornerKey),
            tostring(bridge.regulatedAssemblyId),tostring(bridge.protectedAssemblyId),tostring(bridge.governingPurpose))
        return {status="ACCEPTED",request=request,outcome=outcome,commitment=applied.commitment,candidate=candidate,result=result,actionSpaceRegulation=true,cornerRightOfWay=true}
    end
