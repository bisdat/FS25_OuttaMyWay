...
function Evaluator:enterSettling(commitmentId,governingBasisVerdict)
    ...
    if record.state == "SETTLING" then
        ...
        if #released > 0 then
            record = OuttaMyWay.CommitmentStateMachine.revise(record,{progressActuationOwnership={},obstructionRelocationActuationOwnership={},epoch=self.epochs:next()})
    ...
    local released = self.authorities:releaseForCommitment(commitmentId)
    if #released > 0 then
        updated = OuttaMyWay.CommitmentStateMachine.revise(updated,{progressActuationOwnership={},obstructionRelocationActuationOwnership={},epoch=self.epochs:next()})
        updated = self.commitments:save(updated)
    end
