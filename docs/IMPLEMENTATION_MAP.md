...
| Cooperative Passage Control | `CooperativePassageControl.lua` plus shared mechanisms | Owns six module-local execution/settlement/alignment/watchdog/heartbeat calibrations under [Configuration architecture](CONFIGURATION.md#cooperative-passage-control-implementation-ownership). Validated mechanics, participant-specific handback and lifecycle/safety/feasibility boundaries remain unchanged; traversal-gate semantics remain unresolved. |
...
| Mixed runtime constants | `scripts/config.lua` | **ACTIVE #87** — `.52`-.62 resolve prior ownership/retirement families; `.63` retires the remaining core-capability/control pseudo-state gates. Root identity retirement leaves only `MOD_NAME` and `VERSION`; diagnostic HUD values are now module-local or retired; Cooperative Passage Control owns its six internal calibrations. Resolution-Space reserve/creep policy is module-owned by ResolutionSpaceProgressionEnvelope. Transit settlement derivation and defensive fallback have separate module owners. LocalPassagePlanner owns its nine fixed construction/sweep values. Exactly three non-identity roots remain: the 80 m local Action-Space / entry bound, the 1 m Candidate/Control traversal-radius reuse, and Forward Intersection 1 km/h policy. Their deeper ownership remains unresolved. |
