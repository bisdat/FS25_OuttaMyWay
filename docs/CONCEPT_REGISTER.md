...
| physically relevant entity | Accepted | Entity that can materially affect active work even when it is not an active cooperative worker; current physical relevance does not require historical Operation or Job provenance. | [Runtime §6](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#6-active-participation-and-physical-relevance) |
| Causal Obstruction | Accepted | Current positive relationship in which a physical blocker prevents an active supported beneficiary from continuing; the obstruction predicates relevance and Resolution purpose. | [Runtime §6](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#causal-obstruction) |
| Obstruction Recognition != Actuation Authority | Accepted | Current causal blockage establishes relevance first; blocker classification and Bounded Authority separately determine the permitted response. | [Runtime §6](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#causal-obstruction) |
| Obstruction Relocation | Accepted | Bounded Resolution response for a non-active, unclaimed Causal Obstruction; each actuation moves no farther than the current per-actuation cap toward the Field World centroid and fresh positive obstruction may justify another while meaningful inward space remains. It is geometry-bounded, not move-count-bounded; past Job history and vehicle ownership are not permission prerequisites. | [Runtime §8](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#8-beneficiary-and-controlled-subject) |
| ENDED Job Episode Non-Activity Evidence | Accepted | A positively ENDED GIANTS Job Episode may establish the activity side of non-active blocker classification for that former worker; assemblies without lifecycle evidence instead use current inactivity observation. A new ACTIVE Job Episode is ordinary Situation change, not relocation-specific supersession. | [Runtime §6](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#causal-obstruction) |
| Current Physical Relocation Reference | Accepted | Purpose-scoped current object/pose/positive-only representation evidence for bounded Obstruction Relocation. Coverage is independent of Observation provenance and grants no Situation, semantic, negative-clearance or actuation authority. | [Runtime §6](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#causal-obstruction) |
...
| Pairwise Resolution Exclusivity | Accepted | At most one coupled commitment per Operation; Cooperative Passage requires exactly two active AI participants at admission, while participant-scoped legs may terminalise independently after commitment. | [Runtime §15](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#15-pairwise-resolution-exclusivity) |
| Partial Basis Cessation != Responsibility Termination | Accepted | Loss of support for one subject-scoped obligation does not terminate a wider Resolution while other legitimate obligations remain open. | [Runtime §13](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#resolution-commitment) |
...
| Job Termination Owns Active-Participant Loss | Accepted | While a qualifying GIANTS AI Job Episode remains active, player entry/presence is not a parallel lifecycle authority; active-participant loss is established by authoritative termination of the exact Job Episode. | [Runtime §7](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#7-completion-leaves-occupancy-not-responsibility) |
| Pre-Semantic Contradiction != Resolution Failure | Accepted | A raw Job-token contradiction visible to Control is evidence pending semantic Job Episode reconciliation; it may block new procedural progression but cannot terminalise Resolution or justify player escalation by itself. | [Runtime §7](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#7-completion-leaves-occupancy-not-responsibility) |
| Player Claim Boundary | Accepted | Player entry is presence only while a qualifying GIANTS AI Job Episode remains active; for a non-active blocker, `getIsEntered()==true` is a current human claim requiring hands-off/relinquishment and `false` means unclaimed. | [Runtime §6](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#causal-obstruction) |
...
| Core Capability Has No Enable State | Accepted | Beneath product master enablement, supported core runtime capabilities exist through current Responsibility, Bounded Authority and typed Control topology; they are not independently switched on/off. | [Runtime §17](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#core-capability-availability-is-structural) |
| Architectural Prohibition Has No Disable Flag | Accepted | Unsupported/general Control is absent because no responsibility, authority grant or typed dispatcher path authorises it; a false boolean is not the safety mechanism. | [Runtime §17](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#core-capability-availability-is-structural) |
| Negative Authority Annotation != Capability Enable State | Accepted | Evidence products may explicitly record that they grant no Control authority; such scope metadata is not a mutable capability switch. Field World identity/equivalence evidence is the current example. | [Runtime §17](architecture/RUNTIME_RESPONSIBILITY_ARCHITECTURE.md#core-capability-availability-is-structural) |
...
| Cooperative Passage | Accepted | Pairwise Resolution Commitment admitted by two active AI workers and executed through two participant-scoped Passage Legs. | [Spatial §8](architecture/SPATIAL_NEGOTIATION_MODEL.md#8-cooperative-passage-admission-and-bubble) |
| Committed Responsibility Cannot Be Revoked by a Feature Flag | Accepted | Once Cooperative Passage is committed, only its accepted lifecycle, governing-basis, safety, feasibility or escalation evidence may end/fail it; an implementation switch cannot revoke the Resolution. | [Spatial §9](architecture/SPATIAL_NEGOTIATION_MODEL.md#9-committed-passage-execution) |
| Passage Leg | Accepted | One original participant's already-committed Passage execution and intervention-created debt under the parent coupled Resolution. | [Spatial §9](architecture/SPATIAL_NEGOTIATION_MODEL.md#9-committed-passage-execution) |
| Passage Leg Vacatur | Accepted | Positive loss of one executable AI participant terminalises only that participant's Passage Leg; it does not cancel the surviving leg or admit a replacement. | [Spatial §9](architecture/SPATIAL_NEGOTIATION_MODEL.md#9-committed-passage-execution) |
| Survivor Invariance | Accepted | A surviving Passage Leg continues its existing choreography without a new Candidate, Resolution or survivor mode after the other leg is vacated. | [Spatial §9](architecture/SPATIAL_NEGOTIATION_MODEL.md#9-committed-passage-execution) |
...
| Interaction Bubble / Bubble | Accepted | Coupled Passage context admitted from two active AI workers and retained while at least one original Passage Leg or shared Bubble obligation remains live; no persistent pair history. | [Spatial §8](architecture/SPATIAL_NEGOTIATION_MODEL.md#8-cooperative-passage-admission-and-bubble) |
| Bubble Protection | Accepted | Protects coupled obligations/Epoch from independent spatial consumption while at least one Passage Leg remains live. | [Spatial §8](architecture/SPATIAL_NEGOTIATION_MODEL.md#8-cooperative-passage-admission-and-bubble) |
...
| Last-Leg Dissolution | Accepted | Dissolves immediately when both original Passage Legs are terminal by handback and/or vacatur, without tail, timeout, cooldown or memory. | [Spatial §11](architecture/SPATIAL_NEGOTIATION_MODEL.md#11-last-leg-dissolution) |
