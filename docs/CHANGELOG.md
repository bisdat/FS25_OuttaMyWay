...
TS010 field evidence disproved nearest-boundary Stage 2: the completed assembly's contained target/path could still traverse the held productive worker. **Protected Yield Is Not Clearance.** Stage 2 now uses one deterministic outer-boundary ray away from the authorising productive occupancy and requires continuous current-occupancy clearance for the completed compact DISC sweep. Unsupported means exhaust/fail closed; there is no alternate-boundary or parking search and no third courtesy. No literal or other Passage/terminal behaviour changes.
...
Owner-declared v0.1.7.0 canonical remains authority. Production Cooperative Passage is D-0146-only and Transit-only: missing cached `TRANSIT_BASE` now fails closed rather than falling back to configuration-conditioned planning. D-0143 TS015 production Candidate/Control authority and its runtime literal family are retired; historical donor/test evidence remains. Two still-live D-0146 implementation values (0.25 km/h Hold-effect threshold, 1000 ms heartbeat) are re-owned without tuning. Restoration is deliberately unchanged and remains Closure B.
...
**Candidate behaviour:** cumulative validated v0.1.4.4/v0.1.4.6/v0.1.4.7/v0.1.4.8 behaviour with no new algorithmic change: v0.1.4.0 Passage geometry/timing retained; always attempt Transit at the existing configuration point; required compaction remains strict; inert opportunistic retained-current Transit cannot veto Passage; 1.00 m nominal Crossing-Window target uses a 0.95 m admissible floor while represented non-contact remains hard.

**Regression evidence:** TS009, TS010S, random-start TS010, TS015 and TS016 all completed on v0.1.4.8. The remaining visual caveat is Zero-Development Entry Compression on close straight/zero-deficit passages.

**Rejected branches:** v0.1.4.1-v0.1.4.3 post-compaction replanning and v0.1.4.5 exhaustive Candidate backfill are evidence only and are not active architecture/implementation.

**Open debt:** complex-assembly coverage authority, invalid `allowUnfoldingByAI` XML-path probing, Passage-created agronomic debt/restoration, Restoration Alignment, inherited 8 km/h guide speed and parked literal work.

---

# v0.1.4.8 TEST — Opportunistic Transit Non-Veto

- Keep v0.1.4.7 Passage-clearance band and all v0.1.4.0 timing/guide behaviour.
- Keep `COMPACT_REQUIRED` compaction authoritative and fail-safe.
- Make successful `RETAIN_CURRENT` Transit requests non-vetoing when Reality shows no active fold transition.
- Continue waiting while an optional Transit request is physically transitioning, so useful folding still completes before movement.
- Add bounded `TRANSIT_COMPACTION_OPTIONAL_NONBLOCKING` telemetry.

---

# v0.1.4.7 TEST — Nominal Passage Clearance Policy Band

- Retain 1.00 m nominal Passage construction target.
- Add 0.95 acceptance ratio/floor for Crossing-Window validation.
- Retain hard represented non-contact and D-0165 outside-window semantics.
- Retain v0.1.4.6 clearance telemetry, extended with nominal/floor residues.
- No candidate-memory backfill or extra planner/sweep calls.
...
Owner-declared v4.7.121 remains canonical until explicit owner declaration. v4.7.128 packages the live-validated v4.7.127 behaviour unchanged; changes are candidate identity/provenance, canonical-review records and version assertions only.

The candidate records two successful end-to-end validations of the current D-0147 shape: v4.7.126 first completed the Patriot + Condor + S416 theatre, and v4.7.127 completed fully again after audit alignment. No traffic, Candidate, Commitment, authority, Regulation or Control behavioural algorithm is intentionally changed.

The separately agreed pre-1.0 versioning reset remains deferred until after the owner canonicalises or rejects this exact candidate.

---

# v4.7.127 TEST BUILD — D-0147 Courtesy Constraint Exception / architecture-code audit alignment

Owner-declared v4.7.121 remains the authoritative canonical baseline. v4.7.127 preserves the live-validated v4.7.126 D-0147 movement and closes the agreed architecture/code audit.

- Records the **Three-Assembly Courtesy Continuation Validation** from v4.7.126: Patriot and Condor both completed protected 60 m native-max Bounded Infield Retreats; S416 completed the operation.
- Resolves the **Courtesy Evidence Gap** by defining the **D-0147 Courtesy Constraint Exception**. Generic predictive Field World containment and complete-envelope Transition Clearance are not applicable to this player-consented special-case movement; no synthetic PASS evidence is allowed.
- Keeps all authority/safety/lifecycle contracts mandatory: Terminal Yield Consent, positive Terminal Occupancy, Protected Yield, Player Claim/source supersession, boundedness, Continuation Renewal, Courtesy Exhaustion and Actuation Neutralisation.
- Corrects proven ValueRecord native-length defects found during audit in Candidate/Commitment/D-0146 paths.
- Updates governing current-state documentation while preserving historical records.
- Makes no change to 60 m, native maximum speed, one-shot centroid bearing, fixed-direction actuation, or Protected Yield timing.
...
- Starts from the tested v4.7.107 implementation over owner-declared canonical v4.7.102.
- Records the v4.7.107 live result: five successful D-0146 passages and normal Condor/Patriot Job Episode completion, with one narrow release-chatter defect.
- **Settled Relationship Dissolution:** a trajectory model accepting a new non-opposed direction is not positive relationship invalidation while either participant is still Transitional Continuation.
- Situation requires both participants to have valid `SETTLED_CONTINUATION` local intent plus positive `NON_TURN_LINE_ACTIVE` continuation before the non-opposed-trajectory release path becomes authoritative.
- Physical post-passage ordering remains an immediate positive dissolution path.
- Resolution-Space Regulation therefore persists through TURN_SEGMENT/TURNING trajectory acceptance and releases only on settled positive invalidation or Step-2 succession.
- No passage planner, configuration, admission, speed-cap, or Control geometry changes.
- Adds regression coverage for the exact v4.7.107 release/reacquire pattern and transition-only diagnostics for settled/dissolution state.

---

# v4.7.107 TEST BUILD — Resolution-Space Obligation Persistence

- Starts from canonical v4.7.102 through the live-tested v4.7.106 implementation lineage.
- Records that v4.7.106 correctly admitted early D-0146 Action-Space Regulation but incorrectly released it when the initiating Current Excursion temporarily stopped closing.
- Moves relationship-dissolution semantics into Situation Knowledge as `resolutionSpaceRelationship`; Control consumes only positive invalidation rather than Current Motion details.
- Keeps an admitted Regulation through transient reverse/non-closing and continued Potential conflict; releases on positive relationship dissolution, pair disappearance, or Step-2 succession.
- No headland special case, timeout, route prediction or passage-mechanics change.

---

# v4.7.106 TEST BUILD — D-0146 Potential Conflict Action-Space Conservation

- Starts from owner-declared canonical v4.7.102 (`f85256ddba7cdf4b0be84ef53cc011c0e907237e86dadf137ce2b065a91b597b`; Git `cf51498316714c568b75ee6e65dab544ccbe7af3`; 310 files) through the live-successful v4.7.105 implementation lineage.
- Records v4.7.105 live evidence: no false follower Regulations; two successful S 416/sprayer passages; one successful Condor/Patriot passage; final Condor/Patriot head-on after a Current Excursion consumed Local Passage entry space before trajectory supersession.
- Implements **Excursion-to-Conflict Action-Space Conservation** without weakening Trajectory Persistence.
- One Current Excursion + one stable positively approaching participant + positive current corridor support + positive closure inside the existing 80 m Local Passage envelope can support `POTENTIAL_OPPOSED_CORRIDOR_CONFLICT` and bounded `REGULATE_SPEED` of the stable participant.
- The excursion participant remains GIANTS-native. Regulation releases immediately if the purpose dissolves, or succeeds into the same D-0146 Commitment when Established Conflict becomes supported for Step 2.
- Adds purpose-specific Representation Fitness using the existing Candidate/Constraint contract; no Constraint bypass.
- Leaves the successful optional configuration reduction, Operation-aware third-party Passage Support, and stale D-0141 retirement unchanged.
- Records **Unilateral Passage Execution** as an accepted future least-intervention refinement; not implemented here.

---

# v4.7.105 TEST BUILD — D-0146 Optional Configuration Reduction
...
- Baseline: owner-declared canonical v4.7.99 (`c3f1f6493fd0e44c8447637827989117daa77a20ba80816cf34270484e5eea8d`; 307 files).
- Implements, rather than redesigns, the accepted D-0146 Step-1 contract.
- Situation Assessment now persists Established Trajectory state across observation cycles, distinguishes a Current Excursion, and supersedes history only after sustained coherent contradictory physical travel.
- Situation Assessment classifies Potential / Established Opposed Corridor Conflict from near-collinear opposed established trajectories, mutual facing relation, current closing/stable motion, and positive Supported Corridor Overlap derived from existing cached positive plan-view primitives.
- Positive overlap is categorical (`overlapM > 0`); magnitude remains descriptive and no negative-clearance authority is inferred from absence of overlap.
- Productive/Transitional evidence remains contextual only.
- Missing pose/corridor-anchor evidence fails closed.
- New Knowledge is passive: no Candidate, Decision, Commitment or Control consumer is added; bounded TS015 Cooperative Passage and D-0141 Regulation are preserved.
- Provisional filtering values are implementation calibration for live evidence gathering, not architecture.
- Offline validation: 201 Lua behavioural tests PASS; 78 Python structural/conformance tests PASS. Live GIANTS evidence is the next validation authority.
...
- Owner live validation on 2026-08-10 completed the full TS015 Condor/Patriot working session. Three autonomous Refuge relocations were initiated, including the final role-reversed head-on that exercised same-Commitment supporting-authority reuse and successfully dispatched `REPOSITION`.
- Both workers ultimately completed their GIANTS work. After Patriot completion, the owner manually moved the completed Patriot assembly so Condor could consume the final few metres. This terminal/post-completion occupancy limitation is explicitly parked for later implementation work.
- Retains the validated D-0140 authority alignment and D-0141 follower path: 250 ms live reassessment, 0.90 factor only after Regulation is otherwise required, strict new admission, remote-negative protection, elastic cap updates, outbound-egress lease preservation, Progress Passage retirement and same-Commitment Yield succession.
- Retains existing provisional/test boundaries: Provisional Demand Seed/13 s temporal seed, bounded TS015 Refuge machinery, D-0131/D-0133 shadow, D-0134/D-0136/D-0138 passive, D-0137 falsified, and general production Control disabled.
- Owner-declared v4.7.49 remains canonical until explicit declaration of this exact candidate.
...
# v4.7.73 TEST BUILD — D-0141 Second-Encounter Passage Repair

- Owner-declared v4.7.49 remains canonical. v4.7.73 is a non-canonical implementation repair over v4.7.72.
- v4.7.72 live evidence showed the follower Regulation chain working through Condor's boundary slowdown, but the unscaled cap remained too permissive and the existing D-0141 purpose was positively retired at the exact co-directional → opposed strategy transition (`headingDot=-1`). A head-on Reposition began 251 ms later; with D-0131 still shadow and no sealed Progress horizon, Patriot accelerated under GIANTS during Condor's outbound egress and collided.
- Restores the empirically validated `0.90` Transition-Clearance factor to the **active aligned D-0141 magnitude**, but only after the unscaled calculation already requires Regulation. It cannot manufacture Regulation for an otherwise naturally safe/distant follower.
- Restores D-0130 strategy-succession behaviour in the aligned implementation: an already-admitted follower purpose is preserved through a clean near-opposed continuation instead of being retired by heading reversal alone. The existing speed lease therefore remains active through the subsequent bounded head-on egress.
- During active P22 outbound phases (`TS015_SETTLING`, `TS015_COMPACTING`, `TS015_MOVING`, `TS015_TARGET_COMPACTING`), that existing D-0141 magnitude is preserved unchanged so OuttaMyWay-controlled Yield motion cannot relax the Progress cap before Refuge is established.
- No new egress controller is added. Existing Progress Passage remains the positive retirement event once Yield is compact and Held at Refuge, so Progress is then released back to GIANTS-native passage.
- Strict new follower admission, remote-negative protection, 250 ms Control cadence, zero-command handling, elastic cap updates, D-0123 lease composition and D-0140 authority boundaries are unchanged. D-0131/D-0133 remains shadow.
- Offline validation: 101/101 Python structural/conformance tests, 197/197 Lua behavioural tests, 98/98 non-archive Lua parse checks.
...
- Active follower demand does not consume historical native manoeuvre sweep geometry. D-0140 `NativeManoeuvreObservationSource` remains `representationFitnessForBoundaryDemand=UNRESOLVED`, so long-form Transitional/reposition observations cannot manufacture active follower authority.
- Uses the accepted **Provisional Demand Seed**: observed GIANTS working width is a coarse spatial seed and 13 s is an explicit bounded temporal test mechanic pending better Representation Fitness. Neither is route prediction or Assembly-footprint authority.
...
- Restores D-0130's architectural contract: purpose is sticky through temporary uncertainty while cap magnitude is elastic. The old minimum-ever/tighten-only ratchet and historical 0.90 test factor are not used by the aligned active path.
- D-0139 Progress Passage is positive central retirement evidence for the follower purpose; independent Regulation leases remain composable.
- The legacy follower implementation remains diagnostic shadow; D-0131/D-0133 committed-transition remains shadow. D-0123 Guarded Recovery remains aligned and active. D-0134/D-0136/D-0138 remain passive; D-0137 falsified.
- HUD distinguishes active aligned Regulation from legacy shadow without red/green status encoding.
...
- v4.7.49 remains owner-declared canonical; v4.7.68 is test-only over the retained v4.7.67 D-0138 lineage.
- v4.7.67 live evidence supports `spec_aiFieldWorker.aiDriveParams` as an immediate native drive-command surface, while also falsifying use of the immediate command target as a direct Refuge discriminator.
- The D-0136 `track.active` peer-visibility repair was live-exercised (`compared=1`), exposing a second passive plumbing mismatch: persistent tracks were passed directly to `FieldBoundedFutureSpace.build()`, which expects observation-worker `activeObserved`.
- Adds an explicit persistent-track → observation-worker adapter for settlement reassessment only. It copies current activity, Local Intent, Field World snapshot, pose and Physical Representation; it does not mutate the persistent track or alter Future-Space contracts.
- A settling worker may legitimately remain unresolved because its Local Intent has entered `TURNING`; the repair is successful when `WORKER_NOT_ACTIVE` disappears and the result reflects the actual Local-Intent/representation state.
- No new Decision or Control authority. D-0134 Refuge selection remains unchanged.

## v4.7.67 TEST BUILD — D-0138 Native Field-Worker Drive Command Surface Shadow

- v4.7.49 remains owner-declared canonical; v4.7.67 is test-only.
- D-0137 is recorded as falsified by live evidence plus exact FS25 1.21.1.0 SDK source: `vehicle.aiDriveDirection` / `vehicle.aiDriveTarget` are not the native field-worker continuation command surface.
- Adds passive `NativeFieldWorkerDriveCommandProbe` over `spec_aiFieldWorker.aiDriveParams` only; it never invokes native drive strategies and never changes `driveToPoint` inputs.
- Samples `valid`, `moveForwards`, world target `tX/tY/tZ`, GIANTS-constrained `maxSpeed`, current pose/travel relation, Productive/Local Intent and blocked state.
- D-0134 Refuge shadow output receives descriptive command-target delta/distance only; Candidate ordering remains unchanged.
- D-0136 settlement snapshots the same command surface for subject and active same-Field-World peers; the `track.active` visibility repair remains retained.
- No new Future Space, negative-clearance, Refuge selection, Regulation, Hold, Reposition or Control authority.

## v4.7.66 TEST BUILD — D-0137 Native AI Drive Signal Shadow Probe

- v4.7.49 remains owner-declared canonical; v4.7.66 is test-only.
- Adds passive `NativeAIDriveSignalProbe` for GIANTS-native `vehicle.aiDriveDirection` and `vehicle.aiDriveTarget`; no Courseplay code copied.
- Correlates the raw native signal with Productive Continuation, Local Intent, physical travel, reverse/turn/blocked state and speed.
- Extends D-0134 Refuge shadow output with descriptive native-drive longitudinal/lateral Candidate relationships; selection remains unchanged.
- Captures native drive signals at D-0136 intent settlement.
- Corrects passive D-0136 peer visibility from `otherTrack.activeObserved` to persistent `track.active`, allowing the settlement reassessment to compare active same-Field-World peers.
- No new Future Space, negative-clearance, Refuge selection, Regulation, Hold, Reposition or Control authority.

## v4.7.65 TEST BUILD — D-0136 Intent-Based Residual Settlement Witness

- v4.7.49 remains owner-declared canonical; v4.7.65 is test-only.
- v4.7.64 live evidence disproved complete coarse-cell filling as the settlement criterion: Condor returned productively to its original split-start region and GIANTS changed to a turn/reposition with only 136/185 projected residual cells painted.
- D-0136 measures native intent rather than geometric absolutes. Residual Potential Demand remains coarse; settlement requires Productive re-entry, continued consumption toward the original productive region, representation-fit origin reacquisition, then Productive→`TURN_SEGMENT`.
- `RESIDUAL_GEOMETRICALLY_FILLED` is supporting evidence only and does not retire Potential Demand by itself.
- Settlement reassessment remains passive and cannot regulate, hold, reposition, select Refuge or actuate Control.

## v4.7.64 TEST BUILD — D-0135 Productive Coverage Residual / Residual Settlement

- Adds a passive worker-specific residual derived only from the coherent first-Productive working corridor back to the Field World boundary; unpainted elsewhere remains UNKNOWN.
- Observes later native convergence and positive productive settlement of that residual.
- On settlement, asks existing coarse Future Space whether another active worker already forms a positive field-bounded intersection.
- No action authority is granted. Owner-declared v4.7.49 remains canonical.

## v4.7.63 TEST BUILD — D-0134 Demonstrated Productive Coverage / Refuge Qualification Shadow

- Adds Job-Episode-scoped positive productive-history rasterisation from live work-marker sweeps while Productive Continuation is positive.
- Adds passive Refuge evidence vectors for existing fixture positions and centroid-directed infield shadow positions.
- Reports Field World fit, current Progress Future Space, demonstrated boundary-manoeuvre entry band and Demonstrated Productive Coverage without changing the selected Refuge.
- Unpainted cells are UNKNOWN; productive history does not retire Transitional Demand or establish clearance.
- No headland exclusion, Refuge preference, numeric policy or new Control authority. Owner-declared v4.7.49 remains canonical.

## v4.7.62 TEST BUILD — D-0133 Pre-Handoff Progress Horizon Retention

- retains owner-declared v4.7.49 as canonical authority and keeps D-0132/D-0131/D-0130/D-0129 semantics otherwise intact;
- records that v4.7.61 still missed D-0131 because the live field-bounded Progress horizon disappeared at the exact Reposition-admission sample even though it had been positively available immediately beforehand;
- retains the latest positive Progress field-bound endpoint upstream in the existing follower/maturation evidence path;
- allows admission to seal that retained endpoint only when live evidence is transiently unavailable and the same Job Episode / tracked Local Intent epoch / Productive Continuation basis remains positive;
- fixes the endpoint in world space so Progress movement consumes rather than refreshes the retained horizon;
- changes no D-0131 threat rule or numeric calibration and leaves all parked issues unchanged.

## v4.7.60 TEST BUILD — D-0131 Committed Transition Protection Catch-up

- retains owner-declared v4.7.49 as canonical authority and keeps D-0130/D-0129 behaviour intact;
- adds a bounded committed-egress supporting Regulation lease instead of relying solely on the retained maturation cap;
- requires positive Productive Progress, field-bounded egress-sweep intersection, and a timing witness against the ideal max-speed egress-completion lower bound before applying the stronger lease;
- reuses the existing 1 km/h temporary test literal and least-permissive lease composition; no new numeric policy is added;
- does not treat egress phase alone as authority and does not infer negative clearance from absence of a positive witness;
- leaves Refuge selection, first-Refuge timing, calibration, brute-force remediation and productive/transitional crossing resolution parked.

## v4.7.59 TEST BUILD — D-0130 Purpose-Preserving Regulation Implementation Catch-up

- retains owner-declared v4.7.49 as canonical authority and keeps D-0129 progression-preservation instrumentation passive;
- prevents a live follower Regulation purpose from relaxing its cap merely because the worker has advanced and the same geometry calculation now permits a larger value; fresh values may tighten or maintain the cap;
- keeps the supporting maturation Regulation active when the pair positively matures into opposed continuation instead of releasing solely on Resolution Strategy succession;
- adds composable P22 Regulation leases so independent supporting purposes coexist and the effective physical speed ceiling is the least-permissive active lease;
- changes the existing D-0123 Guarded-Recovery bridge to release only its own lease, restoring any still-live maturation cap instead of erasing/refusing it;
- adds regression tests for tighten-only semantics and composed-lease handoff;
- introduces no new speed/distance literals and leaves Refuge selection, early first-Refuge actuation, calibration/comfort tuning, brute-force Refuge remediation and productive/transitional crossing resolution parked.

## v4.7.58 TEST BUILD — D-0129 Purpose-Bound Progression Preservation Shadow Probe

Passive architectural instrumentation only. Observation now publishes realised progression evidence; Situation Assessment promotes that motion evidence, positive physical primitives, and open-obligation meaning into the immutable Operational Picture. `ProgressionPreservationProbe` records positive represented witness entry distance from a stable purpose/intent baseline and consumed headroom. It grants no negative-clearance, speed, Decision or Control authority and leaves response opportunity unresolved. v4.7.57 behaviour and all parked tuning/refuge questions remain unchanged.
...
- records v4.7.39 restoration-first evidence: both Condor and Patriot were returned approximately toward their pre-egress continuation context, with materially imperfect heading, yet visually reacquired the interrupted lane under the same GIANTS Job Episode; exact pre-egress position/heading restoration is therefore not required by current evidence;
- records supplementary Precision Farming evidence for Patriot: the nitrogen/application view showed no obvious material untreated gap attributable to the OuttaMyWay diversion/rejoin. Precision Farming was enabled solely as an observation instrument for this experiment; normal OuttaMyWay validation remains DLC-free and no architecture depends on it;
- accepts **Native Continuation Restoration** in principle: when OuttaMyWay materially displaces a worker, it should undo enough of its own disturbance to restore a supportable native continuation context before unrestricted GIANTS handback, without reconstructing GIANTS' exact route; the recorded **Rejoin Anchor** is a recovery reference, not an exact-pose mandate;
- records the later Patriot recovery deadlock as a recovery-protection failure rather than a restoration failure: Condor's native diagonal transition consumed Patriot's recovery space while Patriot was returning from refuge;
- accepts **Guarded Recovery** as composition of existing architecture: Yield recovery may proceed while uncertainty about another participant remains affordable, but once authorised its required recovery Action Space is Committed Demand and Traffic Policeman must continue observing the other participant, escalating `Observe → Regulate → Hold` when revealed demand threatens that committed recovery;
- accepts **Protected Progress Alternation**: temporary movement priority may alternate inside one continuing Commitment as the currently protected demand changes. This does not imply one-worker-at-a-time control; concurrent GIANTS movement remains preferred whenever positively supportable;
- accepts **Expedient Manoeuvre Execution**: once a bounded OuttaMyWay egress/recovery manoeuvre is authorised, Control should execute it at the maximum speed positively supportable by current geometry, configuration, sweep and capability. No v4.7.37–v4.7.39 fixture speed literal gains production authority;
- clarifies that initial `PROGRESS` priority is temporary and cannot override later intervention-created recovery obligations, and that successful GIANTS handback does not by itself discharge the traffic Commitment; observation continues until Durable Separation is positively supported;
- leaves the next architectural objective explicit: define the Situation Assessment evidence contract that makes uncertainty unaffordable during Guarded Recovery and therefore exhausts Observe in favour of Regulation/Hold. Production Decision remains passive and `CONTROL_AUTHORITY_ENABLED=false`.

## v4.7.39candidate — TS015 Native Continuation Restoration Comparison

- begins from owner-declared canonical v4.7.34 (`4b8be7f93fe92e1236c2a50d46622ab3bc688e60fbc4ff3fa56b44b7f36bb5c7`, Git `40b70d2adb6811dd1fdf455ae5bb0f6e76cdf372`; 263 files) and carries forward the validated temporary P22/TS015 mechanisms;
- records v4.7.38 direct-refuge-handback evidence: both workers retained the same GIANTS Job Episode, but Condor skipped the interrupted lane remainder and selected another pass that recreated a head-on, while Patriot visually appeared to reacquire its interrupted lane; this establishes **Native Recovery Variability** and disproves any assumption that successful same-Job movement alone predicts a benign recovery trajectory;
- changes only the explicit-command TS015 evidence harness so OuttaMyWay now reverses its own displacement before handback: the settled pre-egress pose/heading is retained as a probe **Rejoin Anchor**, the worker remains compact through refuge Hold and ingress, then returns toward a short forward rejoin reference, settles, restores the original configuration, and only then releases GIANTS;
- reuses the archived empirically successful forward-only rejoin-orientation mechanism when the rejoin target begins behind the compact worker; this is mechanism evidence only and imports no legacy role, refuge, clearance, release or traffic policy;
- retains the 15 km/h egress/ingress fixture ceiling, the archived 5 km/h orientation literal, approximately 10 m forward / 30 m lateral refuge geometry, 20 s refuge Hold and 120 s post-handoff observation solely for this controlled comparison; none has production authority;
- retains failure-safe Hold/restoration, same-source-field refuge sampling, explicit operator selection of Yield and production `CONTROL_AUTHORITY_ENABLED=false`.

## v4.7.38candidate — TS015 Relocation Pace / Watchdog Correction

- begins from owner-declared canonical v4.7.34 (`4b8be7f93fe92e1236c2a50d46622ab3bc688e60fbc4ff3fa56b44b7f36bb5c7`, Git `40b70d2adb6811dd1fdf455ae5bb0f6e76cdf372`; 263 files) and carries forward the temporary v4.7.37 TS015 autonomous native-recovery characterisation harness;
- records the first v4.7.37 live TS015 relocation evidence as **test invalid for autonomous handback**: Condor reached the refuge target fully compact, but the harness incorrectly let the 25 s compact watchdog continue across the long refuge Manoeuvre Leg and therefore retained Hold instead of releasing GIANTS; explicit `cancel`/`release` subsequently demonstrated same-Job GIANTS continuation, so the observed non-resume was not GIANTS recovery evidence;
- corrects watchdog scope: fold-motion evidence is still required before movement; positive full-compaction observed during movement retires the initial compaction watchdog, and any endpoint compact wait is measured from target arrival rather than from the original fold request;
- changes only the autonomous TS015 fixture relocation ceiling from 5 km/h to 15 km/h because live video showed the 5 km/h crawl materially distorted the traffic experiment. This remains a probe literal only and grants no production reposition-speed policy or threshold authority;
- retains the same approximately 10 m forward / 30 m lateral fixture target, compact refuge dwell, timed handback and 120 s hands-off observation;
- retains production `CONTROL_AUTHORITY_ENABLED=false`; no Situation Assessment, Candidate, Decision, Commitment, production role-selection, Refuge Region, Safe Release or speed-policy authority is added.

## v4.7.37candidate — TS015 Autonomous Native-Recovery Characterisation Harness

- begins from owner-declared canonical v4.7.34 (`4b8be7f93fe92e1236c2a50d46622ab3bc688e60fbc4ff3fa56b44b7f36bb5c7`, Git `40b70d2adb6811dd1fdf455ae5bb0f6e76cdf372`; 263 files) and carries forward the validated temporary Prototype 22 capability mechanisms;
- records revised P22-C live evidence for both Condor Endurance II and Patriot 4450: folding overlapped bounded movement, represented maximum plan-view span reduced materially at the target, original configuration was restored and the same GIANTS Job Episode resumed;
- adds explicit-command `otmP22 relocate <Condor|Patriot>` orchestration for TS015 native-recovery characterisation: Hold → compact → literal refuge leg → compact Hold → restore/unfold while Held → fixture-timed handback → 120-second hands-off GIANTS observation;
- uses probe-only literals of 10 m forward, 30 m lateral, 5 km/h, 8 s compact-refuge dwell and at least 20 s total refuge Hold. These values grant no production Refuge Region, passage, Durable Separation or Safe Release authority;
- requires exactly two active GIANTS AI field workers and selects the non-commanded worker only as the fixture Progress participant; no production role-selection policy is implemented;
- qualifies the literal target only by same-source-field sampled support and explicitly leaves obstacle/fixture clearance operator-owned; failure retains Hold and attempts configuration restoration rather than blind release;
- extends post-release observation rather than declaring PASS on first native motion, so GIANTS work recovery, return/repetition and near-term Encounter recurrence can be characterised from logs/video;
- records the architectural refinements discovered before the probe: qualified Refuge Regions precede Decision, Reposition uses bounded Reachability Progress without a fixed Manoeuvre-Leg count, committed Reposition strategy persists until materially invalidated, and Return Potential / Encounter Recurrence Potential / Durable Separation inform role and release reasoning;
- retains production `CONTROL_AUTHORITY_ENABLED=false`; the harness cannot arm from Situation Assessment, Candidate generation, Decision or Commitment.

## v4.7.36candidate — Prototype 22 Spatial Reposition / Configuration Evidence Correction

- begins from owner-declared canonical v4.7.34 (`4b8be7f93fe92e1236c2a50d46622ab3bc688e60fbc4ff3fa56b44b7f36bb5c7`, Git `40b70d2adb6811dd1fdf455ae5bb0f6e76cdf372`; 263 files) and reapplies the v4.7.35 Prototype 22 capability-gate delta plus the live-evidence correction;
- records v4.7.35 live evidence: Regulation and stable-state Hold/release passed for both Condor Endurance II and Patriot 4450; the bounded forward target actuator also reached operator-selected targets and returned both same Job Episodes to GIANTS;
- corrects P22-C after visual evidence showed neither 36 m sprayer boom folded: target actuation alone is no longer accepted as spatial Reposition evidence;
- adds temporary configuration authority for P22-C only: work-off/raise/fold requests, explicit ownership of those mutations, restoration before same-Job handback, and safe cancel-to-Hold restoration;
- permits folding to overlap the outbound forward Manoeuvre Leg after actual fold motion is observed, preserving the potentially valuable ~15 s configuration interval without making a numerical fold-progress threshold spatial authority;
- requires P22-C spatial PASS to combine full compact configuration with positive reduction in the realised represented plan-view hull span; representation unable to prove reduction yields `UNRESOLVED`, not PASS;
- adds transition-only P22 HUD signalling and one-shot final `SUMMARY` records so the operator no longer has to follow saturated raw console output during live testing;
- retains all v4.7.34 architectural boundaries: production Decision remains passive, `CONTROL_AUTHORITY_ENABLED=false`, no role/refuge/target-clearance/Safe-Release policy is introduced, and OuttaMyWay-directed reverse remains `UNRESOLVED`.

## v4.7.35candidate — Prototype 22 Traffic Policeman Capability Gate

- begins from owner-declared canonical v4.7.34 (`4b8be7f93fe92e1236c2a50d46622ab3bc688e60fbc4ff3fa56b44b7f36bb5c7`, Git `40b70d2adb6811dd1fdf455ae5bb0f6e76cdf372`; 263 files);
- adds manual-only Prototype 22 capability validation for bounded GIANTS-native Regulation, same-Job Hold/release and participant-complete one-leg forward Reposition;
- reuses only empirically proven archived GIANTS integration points: the transparent field-worker continuation permission gate and a scoped final `AIVehicleUtil.driveToPoint` wrapper; no legacy architecture or fixture policy is restored;
- keeps production Decision passive and `CONTROL_AUTHORITY_ENABLED=false`; P22 cannot arm automatically and requires an explicit `otmP22` console command with at least two active GIANTS AI field workers;
- Regulation preserves GIANTS steering and forward/reverse ownership while bounding only the passed speed ceiling;
- Hold is capability evidence only and is tested at operator-confirmed stable states; it does not grant arbitrary Transitional interruption authority;
- forward Reposition uses one manually requested bounded `moveForwards=true` Manoeuvre Leg, retains the permission Hold during the leg and ends Held at the target until manual release; target/refuge clearance remains operator-owned for this capability probe;
- explicitly refuses negative forward offsets as `REVERSE_REPOSITION_UNRESOLVED`; reverse remains architecturally valid and GIANTS-native reverse during Regulation remains permitted;
- adds same-Job identity monitoring, manual release/cancel, post-release native-continuation monitoring and a dedicated live protocol at `docs/prototypes/PROTOTYPE_22_TRAFFIC_POLICEMAN_CAPABILITY_GATE.md`.
...
- Establishes the clean v4.7.x implementation boundary from canonical v4.6.78.
- Preserves all v4.6.78 Lua bytes in a non-executable archive.
- Implements the inert canonical contract/lifecycle/obligation/authority kernel.
- Adds offline conformance and archive-independence validation.
- No GIANTS Observation or Control authority is present.
