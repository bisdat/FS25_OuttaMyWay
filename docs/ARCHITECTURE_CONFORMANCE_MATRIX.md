...
|---|---|---|---|
| Runtime behaviour | consolidation must not silently implement production Traffic Policeman | P22/TS015 runtime modules carried forward from v4.7.39; `CONTROL_AUTHORITY_ENABLED=false` | PRESERVED |
| Restoration responsibility | reverse OTM-created displacement without route ownership | Native Continuation Restoration + Rejoin Anchor recorded as approximate continuation-context restoration, not exact routing | ACCEPTED IN PRINCIPLE |
| Agronomic evidence | supplementary evidence must not become dependency | Patriot PF overlay recorded as supplementary only; normal validation remains DLC-free | PRESERVED |
| Recovery protection | authorised ingress must remain protected | recovery Action Space becomes Committed Demand; Guarded Recovery retains Observe→Regulate→Hold protection | ARCHITECTURE ACCEPTED / IMPLEMENTATION OPEN |
| Temporary priority | roles must not fossilise | Protected Progress Alternation permits priority change within one Commitment; concurrent movement remains preferred | ACCEPTED |
| Manoeuvre pace | minimum authority must not imply timid execution | maximum supportable speed principle recorded; no numeric speed policy | ACCEPTED / IMPLEMENTATION OPEN |
| Handover semantics | native movement alone must not terminate traffic responsibility | post-handback observation continues until Durable Separation is supported | ACCEPTED |
| Next implementation gate | evidence before actuation | define SA evidence for threatened committed recovery and Observe exhaustion before production changes | OPEN |

---

# v4.7.36 Prototype 22 Spatial Reposition / Configuration Evidence Conformance

| Concern | Required boundary | v4.7.36 implementation | Status |
|---|---|---|---|
| Architecture stability | live correction must not redefine Traffic Policeman | D-0117 is probe/validation correction only; canonical v4.7.34 unchanged | PRESERVED |
| Regulation / Hold | preserve v4.7.35 positive evidence | mechanisms retained unchanged apart from HUD/summary signalling | LIVE PASS / REGRESSION OPTIONAL |
| Spatial Reposition | target actuation alone is insufficient | P22-C requires compact configuration and positive represented hull-span reduction | AWAIT LIVE |
| Configuration latency | implementation must not assume folding time is dead time | forward Manoeuvre Leg begins after actual fold motion is observed; folding may continue during movement | AWAIT LIVE |
| Fold number authority | profile/fold values must not become spatial clearance | fold values are diagnostic; spatial PASS separately requires realised representation reduction | PRESERVED |
| Restoration | P22-owned mutations must settle before handback | unfold/lower/work restoration occurs while Hold remains active, then same-Job release monitor | AWAIT LIVE |
| Operator diagnostics | live tester must not depend on saturated console | transition-only text HUD + one-shot SUMMARY records | PASS offline |
| Reverse boundary | reverse valid but unsupported active authority must not execute | negative forward offset still refused as `REVERSE_REPOSITION_UNRESOLVED` | PRESERVED |
| Production authority | probe must remain isolated | explicit console arming only; `CONTROL_AUTHORITY_ENABLED=false` | PASS offline |

# v4.7.35 Prototype 22 Traffic Policeman Capability-Gate Conformance

| Concern | Canonical requirement / gate | v4.7.35 implementation | Status |
|---|---|---|---|
| Architecture stability | capability validation must not redefine D-0115 | no Traffic Policeman policy changes; D-0116 is implementation-only | PRESERVED |
| Manual isolation | production Decision must not acquire probe authority | `otmP22` explicit arming only; no Situation/Decision/Commitment connection; `CONTROL_AUTHORITY_ENABLED=false` | PASS offline |
| Regulation ownership | GIANTS must retain route/steering/direction | P22 changes only final `maxSpeed` for selected subject | AWAIT LIVE |
| Hold semantics | temporary wait must preserve Job Episode | proven continuation-permission overwrite reused only for selected subject | AWAIT LIVE |
| Reposition scope | capability gate must not smuggle refuge policy | operator-selected roomy target; one bounded `moveForwards=true` leg; no target-clearance claim | AWAIT LIVE |
| Participant completeness | fixed Condor-as-Yield behaviour forbidden | subject chosen manually from any active worker; Condor and Patriot both required for full live PASS | AWAIT LIVE |
| Reverse boundary | reverse remains valid but unsupported actuation must not execute | negative forward offset refused as `REVERSE_REPOSITION_UNRESOLVED`; GIANTS-native reverse under Regulation untouched | PRESERVED |
| Job identity | probe must not restart/replace a changed Job Episode | token monitored continuously; authority removed on identity change; release monitor checks same-job continuation | AWAIT LIVE |
