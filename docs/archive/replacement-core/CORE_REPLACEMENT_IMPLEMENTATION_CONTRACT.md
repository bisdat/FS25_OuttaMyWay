...
**D-0147 explicit architecture exception:** the Courtesy Constraint Exception remains a governing architecture decision for player-consented Interior Settlement: generic predictive `FIELD_WORLD_CONTAINMENT` and complete-envelope `TRANSITION_CLEARANCE` are non-applicable there. D-0196 narrows the exception for Final Boundary Settlement: `TRANSITION_CLEARANCE` becomes applicable for the one deterministic move against the authorising productive worker's **current positive physical occupancy**; this grants no future-demand exclusion, route or parking authority. All other mandatory constraints remain applicable. Normal physical Candidates retain the full mandatory proof contract.
