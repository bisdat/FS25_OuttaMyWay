## D-0223 — Validated Plateau Versioning and the 0.4 Architectural Epoch
...
**Owner plateau selection — 2026-09-25:** field-validated TEST `0.4.1.4` is selected as the next Validated Plateau and promoted by a behaviour-neutral checkpoint increment to **`0.4.2.0`**. The tranche implements and validates the existing Configuration and Log Publication architecture rather than creating a new architecture/capability epoch. `0.4.2.0` therefore advances Accepted Repository State as a PATCH checkpoint and remains non-canonical; canonical remains `0.4.0.0`.
