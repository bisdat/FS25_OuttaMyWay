...
| `vehicle:getAIReverserNode()` | AI-drivable vehicle; method | Returns the reference node current FS25 `AIFieldWorker` uses to transform reverse world targets before `driveToPoint()`. | Reverse coordinate-frame surface only. It does not choose the target, prove clearance, or grant path authority. | Current FS25 source; Issue #336 investigation |
| `AIVehicleUtil.getAIToolReverserDirectionNode(vehicle)` | `AIVehicleUtil`; helper | Recursively returns an attached AI tool's reverser-direction node when exposed. Current `AIDriveStrategyFieldCourse` uses it both as reverse course reference context and, separately, in a post-target lateral/longitudinal transform before `AIFieldWorker` converts the adjusted world point through `getAIReverserNode()`. | Positive tool reverse-geometry surface. Reusing the post-target transform for an already-authorised point objective does not import field-course routing; presence still proves neither clearance nor route authority. | Current FS25 source; Issue #336 investigation |
