...
| `AIVehicleUtil.getAIToolReverserDirectionNode(vehicle)` | `AIVehicleUtil`; helper | Recursively returns an attached AI tool's reverser-direction node when exposed; current `AIDriveStrategyFieldCourse` retains it for reverse course geometry. | Positive tool reverse-geometry surface. Presence does not itself prescribe an OuttaMyWay target transform; use remains purpose-bounded and evidence-led. | Current FS25 source; Issue #336 investigation |
