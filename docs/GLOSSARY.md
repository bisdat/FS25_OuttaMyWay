...
Recovery responsibility created when OuttaMyWay materially displaces a worker. Before unrestricted GIANTS handback, OuttaMyWay should restore a supportable state derived from the positively observed pre-intervention continuation context where current traffic obligations permit it. This reverses OuttaMyWay's own disturbance; it does not reconstruct GIANTS' exact route or require exact-pose replay.

**Rejoin Anchor**  
The retained positively observed pre-egress position/orientation context used as a reference for Native Continuation Restoration. It is evidence provenance for recovery, not a mandatory exact waypoint, exact heading or lane-routing authority.

**Guarded Recovery**  
A composition of existing Traffic Policeman, Committed Demand and Bounded Observation architecture in which a displaced participant may begin recovery while another participant's intent remains uncertain, provided current recovery Action Space is supportable and remaining uncertainty is affordable because sufficient protective authority remains. Once authorised, the recovery Action Space is Committed Demand and the other participant remains subject to `Observe → Regulate → Hold` as its revealed demand develops.

**Protected Progress Alternation**  
Legitimate alternation of temporary movement priority within one continuing Commitment as the currently protected demand changes. It may colloquially resemble “pause A, move B; pause B, move A”, but is not scripted choreography or a one-worker-at-a-time rule. Concurrent movement remains preferred when compatible; alternation without obligation progress is Revelation Oscillation.

**Expedient Manoeuvre Execution**  
Control principle that an already-authorised bounded OuttaMyWay manoeuvre should be executed at the greatest speed positively supportable by current geometry, configuration, transition sweep and proven capability. Minimum Necessary Authority constrains the scope of intervention, not the decisiveness of execution. No fixed speed is implied.
...
**Refuge Region** — qualified bounded world-space Knowledge where a complete Physical Assembly could potentially establish supportable stationary refuge occupancy under stated Refuge Configuration and representation assumptions. It is not a role, instruction, exact pose or reachability proof.

**Refuge Configuration** — the proposed complete-assembly configuration for stationary refuge occupancy; normally a positively supported raised/folded transit configuration where that equipment provides one. Transition Sweep remains authoritative for intermediate geometry.

**Refuge Pose** — a particular complete-assembly position plus orientation inside a Refuge Region under the proposed Refuge Configuration.

**Lateral Refuge Candidate** — historical/specific lateral refuge-region family on either side of the Protected Progress Corridor. General architecture uses **Refuge Region** so `Candidate` remains reserved for Candidate Actions.

**Refuge Region Qualification** — Situation Assessment filtering that prevents intrinsically unusable world-space from being published as refuge Knowledge and records relational/assembly-conditioned fit evidence without selecting an action.

**Clearance-First, Cost-Second Refuge Selection — refined** — mandatory sufficiency/admissibility is not traded against cost. Among surviving actions, Governing Purpose and Resulting Situation quality precede Minimum Necessary Authority; clearance reserve contributes robustness rather than an objective to maximise.
...
### Manoeuvre Leg
A bounded, coherent physical movement executed under one authorised target until a Settled Movement Boundary or separately authorised emergency interruption. There is no fixed architectural maximum count; another leg requires fresh support for Reachability Progress or another unresolved Commitment obligation.

### Reachability Progress
Positive improvement in the supported relationship between the current assembly state and the governing Refuge Region/Commitment objective. It may improve access, orientation, transition support, protected-demand compatibility or remaining Action Space and need not reduce straight-line distance.
...
### Return Potential
Supported possibility that a presently departing participant's continuing native intent will bring its demand back into relevant Situation Space before outstanding traffic obligations are discharged.

### Encounter Recurrence Potential
Supported possibility that a proposed traffic resolution will shortly recreate materially the same traffic Situation before Durable Separation can be established.

### Durable Separation
A resulting traffic condition in which supported separation is sufficient, relative to outstanding recovery obligations, for the current intervention to unwind without materially recreating the governed conflict. It is not a fixed distance/time and is distinct from physical passage or immediate recession.

### Reposition Strategy Persistence
After Decision authorises movement toward a selected Refuge Region, the strategy has a presumption of continuation while admissibility, Reachability Progress and governing obligations remain intact. Settled reassessment tests validity; it does not automatically re-optimise every alternative.
